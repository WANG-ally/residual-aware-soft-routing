#!/usr/bin/env python3
"""Weather Daily statistical analysis from saved metrics only.

Raw/processed time series, per-timestamp predictions, and trained models are
never opened. The analysis unit is one of the 500 registered Weather series,
so variable-length series receive equal statistical weight."""
...
EXPECTED_SERIES = 500
EXPECTED_METHODS = 14
EXPECTED_FORMAL_TEST_ROWS = 1_105_774
EXPECTED_TEST_LENGTH_RANGE = (200, 8_802)
EXPECTED_TEST_RELATIVE_STEPS = 8_802
EXPECTED_TYPE_COUNTS = {"maxtemp": 124, "mintemp": 124, "rain": 121, "solar": 131}
EXPECTED_EVALUATOR_HASH = (
    "87d8781850a74ad711273e26c4f607ad6c28377901d9fd1cdbf27da8a6233b0a"
)
EXPECTED_AUTHORIZATION_ID = (
    "f30c7263b9eca0af2d6a34bd102a9c0f131f9645ba76b7b8c68ec098a2fcff35"
)
EXPECTED_FINAL_FREEZE_ID = (
    "004d19955e8b92b9b4fd3c9ee43732a75cf5d0a48bfe1bfd828bb13113c15cf1"
)

RECEIPT_PATH = PROJECT_ROOT / "logs/weather_daily_formal_test_access_receipt.json"
AUTHORIZATION_PATH = PROJECT_ROOT / "logs/weather_daily_evaluator_authorization.json"
EVALUATOR_PATH = PROJECT_ROOT / "code/publication_pipeline/weather_daily/formal_test.py"
PER_SERIES_PATH = PROJECT_ROOT / "results/weather_daily_test_per_series_metrics.csv"
AGGREGATE_INPUT_PATH = PROJECT_ROOT / "results/weather_daily_test_aggregate_metrics.csv"
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"

CI_PATH = OUTPUT_ROOT / "results/weather_daily_method_bootstrap_ci.csv"
PAIRWISE_PATH = OUTPUT_ROOT / "results/weather_daily_primary_pairwise_tests.csv"
NONINFERIORITY_PATH = OUTPUT_ROOT / "results/weather_daily_noninferiority_tests.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/weather_daily_statistical_summary.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/weather_daily_statistical_analysis_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/weather_daily_statistical_comparison.png"
REPORT_PATH = OUTPUT_ROOT / "logs/weather_daily_statistical_analysis_report.json"
...
def main() -> None:
    ...
    formal_identity_valid = bool(
        ...
        and receipt.get("sample_id") == SAMPLE_ID
        and receipt.get("status") == "COMPLETED"
        and int(receipt.get("formal_test_run_number", -1)) == 1
        and int(receipt.get("formal_test_rows", -1)) == EXPECTED_FORMAL_TEST_ROWS
        and int(receipt.get("series_count", -1)) == EXPECTED_SERIES
        and int(receipt.get("relative_time_points", -1))
        == EXPECTED_TEST_RELATIVE_STEPS
        and int(receipt.get("minimum_active_series", -1)) == 1
        and int(receipt.get("maximum_active_series", -1)) == EXPECTED_SERIES
        ...
        and authorization.get("dataset_id") == DATASET_ID
        and authorization.get("sample_id") == SAMPLE_ID
    ...
    # These are the only two formal-result tables parsed by this script.
    per_series = pd.read_csv(PER_SERIES_PATH)
    aggregate_input = pd.read_csv(AGGREGATE_INPUT_PATH)
    series_metadata = per_series[
        ["series_id", "series_type", "test_rows"]
    ].drop_duplicates()
    observed_type_counts = {
        str(key): int(value)
        for key, value in series_metadata["series_type"].value_counts().items()
    }
    per_series_identity_valid = bool(
        ...
        and set(per_series["dataset_id"].astype(str)) == {DATASET_ID}
        and set(per_series["sample_id"].astype(str)) == {SAMPLE_ID}
        and not per_series.duplicated(["series_id", "method"]).any()
        and per_series.groupby("series_id", observed=True)["test_rows"]
        .nunique().eq(1).all()
        and per_series.groupby("series_id", observed=True)["series_type"]
        .nunique().eq(1).all()
        and len(series_metadata) == EXPECTED_SERIES
        and int(series_metadata["test_rows"].min()) == EXPECTED_TEST_LENGTH_RANGE[0]
        and int(series_metadata["test_rows"].max()) == EXPECTED_TEST_LENGTH_RANGE[1]
        and int(series_metadata["test_rows"].sum()) == EXPECTED_FORMAL_TEST_ROWS
        and observed_type_counts == EXPECTED_TYPE_COUNTS
    )
    record_check(
        "paired_variable_length_series_table_complete",
        per_series_identity_valid,
        (
            f"rows={len(per_series)}; series={per_series['series_id'].nunique()}; "
            f"methods={per_series['method'].nunique()}; lengths="
            f"{series_metadata['test_rows'].min()}-{series_metadata['test_rows'].max()}"
        ),
    )
    ...
    record_check("RMSSE_pairing_matrix_complete", paired_matrix_valid, f"shape={wide.shape}")
    methods = list(wide.columns)

    aggregate_identity_valid = bool(
        len(aggregate_input) == EXPECTED_METHODS
        and set(aggregate_input["dataset_id"].astype(str)) == {DATASET_ID}
        and set(aggregate_input["sample_id"].astype(str)) == {SAMPLE_ID}
    ...
    record_check(
        "aggregate_equal_series_mean_RMSSE_reproduced",
    ...
    for comparator in methods:
        ...
        statistic, p_value = safe_wilcoxon(primary_values, comparator_values)
    ...
    for comparator in noninferiority_comparators:
        comparator_values = wide[comparator].to_numpy(dtype=np.float64)
        margin_adjusted = (1.0 + noninferiority_margin) * comparator_values
        statistic, p_value = safe_wilcoxon(
            primary_values, margin_adjusted, alternative="less"
        )
        adjusted_difference = primary_values - margin_adjusted
        ...
        noninferiority_records.append(
            {
                ...
                "observed_relative_difference": (
                    float(np.mean(primary_values)) / float(np.mean(comparator_values))
                    - 1.0
                ),
    ...
    record_check(
        "formal_analysis_did_not_reopen_raw_test_predictions_or_models",
        True,
        (
            "only receipt, authorization, config, per-series metrics, and "
            "aggregate metrics parsed"
        ),
    )
    ...
    statistical_summary = {
        ...
        "series_count": series_count,
        "series_type_counts": EXPECTED_TYPE_COUNTS,
        "series_length_structure": "variable test length from 200 to 8802 days",
        "series_weighting": "equal weight per series",
        "primary_method": primary_method,
        ...
        "scientific_warning": (
            "Do not rerun, retune, replace sampled series, exclude unfavorable "
            "comparisons, or claim unsupported superiority"
        ),
    }
    ...
    if not failed_checks.empty:
        raise AssertionError(
            "Weather Daily statistical checks failed: "
            f"{failed_checks.to_dict('records')}"
        )
    ...
    for comparator, color in (
        ...
        axes[2].hist(difference, bins=20, alpha=0.38, label=comparator, color=color)
    ...
    print()
    print("Weather Daily 正式测试统计分析全部通过")
    ...
    print("覆盖率约束是否通过：", "是" if coverage_constraint_passed else "否")
