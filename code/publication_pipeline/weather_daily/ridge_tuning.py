#!/usr/bin/env python3
"""Base-train-only rolling validation for Weather Daily Ridge AR."""
...
LOWER_GRID_EXTENSION = [0.0, 1e-8, 1e-7, 1e-6, 1e-5]
N_FOLDS = 3
SEASONAL_PERIOD = 7
MAX_VALIDATION_SIZE = 56
VALIDATION_BLOCK_UNIT = 7
MIN_INITIAL_TARGETS_AT_MAX_WINDOW = 7

DATA_PATH = PROJECT_ROOT / "data/processed/weather_daily_long.parquet"
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"
WINDOW_SUMMARY_PATH = (
    PROJECT_ROOT / "results/weather_daily_window_preparation_summary.yaml"
)
WINDOW_CHECKS_PATH = (
    PROJECT_ROOT / "results/weather_daily_window_preparation_checks.csv"
)
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/weather_daily_split_summary.yaml"

DETAIL_PATH = OUTPUT_ROOT / "results/weather_daily_ridge_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/weather_daily_ridge_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/weather_daily_selected_ridge_params.yaml"
FOLD_PATH = OUTPUT_ROOT / "results/weather_daily_ridge_fold_manifest.csv"
FOLD_SCALER_PATH = OUTPUT_ROOT / "results/weather_daily_ridge_fold_scalers.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/weather_daily_ridge_tuning_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/weather_daily_ridge_tuning.png"
REPORT_PATH = OUTPUT_ROOT / "logs/weather_daily_ridge_tuning_report.json"
...
def build_fold_schedule(
    ...
    for series_id, values in series_values.items():
        n = len(values)
        available = n - maximum_window - MIN_INITIAL_TARGETS_AT_MAX_WINDOW
        whole_blocks = available // (N_FOLDS * VALIDATION_BLOCK_UNIT)
        validation_size = min(
            MAX_VALIDATION_SIZE, int(whole_blocks * VALIDATION_BLOCK_UNIT)
        )
        if validation_size < VALIDATION_BLOCK_UNIT:
            raise AssertionError(f"{series_id} cannot support {N_FOLDS} folds")
...
def sufficient_statistics_for_windows(
    ...
    """Compute exact X'X and X'y without retaining a dense lag matrix."""
...
def explicit_window_matrix(
    values: np.ndarray, window: int, end: int
) -> tuple[np.ndarray, np.ndarray]:
    values = np.asarray(values[:end], dtype=np.float64)
...
def main() -> None:
    ...
    alphas = LOWER_GRID_EXTENSION + registered_alphas
    if windows != EXPECTED_WINDOWS:
        raise AssertionError(f"Unexpected daily windows: {windows}")
    if (
        len(registered_alphas) != 9
        or registered_alphas[0] != 0.0001
        or registered_alphas[-1] != 10_000.0
        or len(alphas) != 14
    ...
    if base_data.empty or observed_splits != {"base_train"}:
        raise AssertionError(
            f"Tuning input must contain only base_train, got {observed_splits}"
        )
    ...
    for row in fold_manifest.itertuples(index=False):
        ...
        seasonal_differences = (
            training[SEASONAL_PERIOD:] - training[:-SEASONAL_PERIOD]
        )
        denominator = float(np.mean(seasonal_differences**2))
    ...
    audit_coefficient, audit_intercept = solve_ridge_from_statistics(
        audit_statistics, 1.0
    )
    sklearn_audit = Ridge(alpha=1.0, fit_intercept=True, solver="cholesky")
    sklearn_audit.fit(explicit_x, explicit_y)
    coefficient_max_difference = float(
        np.max(np.abs(audit_coefficient - sklearn_audit.coef_))
    )
    intercept_difference = float(abs(audit_intercept - sklearn_audit.intercept_))
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        for series_id, raw_values in series_values.items():
            ...
            for window in windows:
                add_statistics(
                    aggregate_statistics[window], series_statistics[window]
                )
                view = sliding_window_view(scaled_values, window)
                validation_x = view[
                    validation_start - window : validation_end - window, ::-1
                ].copy()
                validation_y_raw = raw_values[
        ...
        for window in windows:
            ...
            validation_denominators = np.asarray(
                buffer["denominators"], dtype=np.float64
            )
            ...
            for alpha in alphas:
                ...
                series_rmsse = np.asarray(
                    [
                        np.sqrt(
                            np.mean(squared_error[offsets[i] : offsets[i + 1]])
                            / validation_denominators[i]
                        )
                        for i in range(len(validation_lengths))
                    ],
                    dtype=np.float64,
                )
                records.append(
                    {
                        ...
                        "validation_start_min": int(
                            fold_rows["validation_start"].min()
                        ),
                        "validation_start_max": int(
                            fold_rows["validation_start"].max()
                        ),
                        "window": window,
                        "alpha": alpha,
        ...
        print(
            f"[完成] fold={fold}/{N_FOLDS}, windows={len(windows)}, "
            f"alphas={len(alphas)}"
        )
    ...
    validation_sizes_valid = bool(
        fold_manifest["validation_count"].eq(EXPECTED_VALIDATION_SIZE).all()
    )
    expected_records = N_FOLDS * len(windows) * len(alphas)
    check_items: list[tuple[str, bool, str]] = [
        (
            "previous_window_preparation_and_sample_id_valid",
            bool(window_summary.get("window_preparation_passed"))
            and window_summary.get("sample_id") == EXPECTED_SAMPLE_ID,
            f"sample_id={window_summary.get('sample_id')}",
        ),
        (
            "all_previous_window_checks_passed",
            len(window_checks) == 18 and bool_series(window_checks["passed"]).all(),
            f"passed={int(bool_series(window_checks['passed']).sum())}/{len(window_checks)}",
        ),
        (
        ...
        (
            "lower_grid_extension_is_fixed_and_base_only",
            alphas == LOWER_GRID_EXTENSION + registered_alphas
            and len(alphas) == 14,
            f"registered={registered_alphas}; extension={LOWER_GRID_EXTENSION}",
        ),
        (
            "base_row_count_matches_frozen_split",
            len(base_data) == EXPECTED_BASE_ROWS
            and len(base_data) == int(split_summary["aggregate_counts"]["base_train"]),
            f"rows={len(base_data)}",
        ),
        (
            "all_500_unequal_length_series_are_present",
            len(series_values) == EXPECTED_SERIES
            and len(series_values) == int(split_summary["series_count"]),
            (
                f"series={len(series_values)}; length_range="
                f"{min(map(len, series_values.values()))}.."
                f"{max(map(len, series_values.values()))}"
            ),
        ),
        (
        ...
        (
            "fold_schedule_is_strictly_chronological",
        ...
        (
            "all_validation_blocks_are_56_days",
            validation_sizes_valid,
            (
                f"range={fold_manifest['validation_count'].min()}.."
                f"{fold_manifest['validation_count'].max()}"
            ),
        ),
        (
        ...
        (
            "all_fold_RMSSE_denominators_are_positive",
        ...
        (
            ...
            coefficient_max_difference < 1e-8,
            f"max_abs_difference={coefficient_max_difference:.3e}",
        ),
        (
            "ridge_intercept_matches_sklearn",
            intercept_difference < 1e-8,
            f"abs_difference={intercept_difference:.3e}",
        ),
        (
            "expected_number_of_fold_parameter_evaluations",
            len(details) == expected_records,
            f"actual={len(details)}; expected={expected_records}",
        ),
        (
            "every_parameter_combination_has_three_folds",
            len(tuning_summary) == len(windows) * len(alphas)
            and tuning_summary["folds"].eq(N_FOLDS).all(),
            f"combinations={len(tuning_summary)}",
        ),
        (
            "selected_alpha_has_no_unresolved_lower_boundary",
            float(best["alpha"]) != max(alphas)
            and (
                float(best["alpha"]) != min(alphas)
                or float(best["alpha"]) == 0.0
            ),
            (
                f"selected={float(best['alpha'])}; natural_lower_endpoint=0; "
                f"upper={max(alphas)}"
            ),
        ),
        (
            ...
            np.isfinite(
                details[["mean_series_RMSSE", "median_series_RMSSE"]].to_numpy(
                    dtype=float
                )
            ).all(),
        ...
        (
            ...
            True,
            "Parquet predicate loaded base_train only",
        ),
    ]
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Weather Daily Ridge tuning failed: {message}")
    ...
    selected = {
        ...
        "validation_schedule": {
            ...
            "minimum_initial_targets_at_max_window": (
                MIN_INITIAL_TARGETS_AT_MAX_WINDOW
            ),
            ...
            "rule": (
                "Largest whole-week block up to 56 days permitting three expanding "
                "folds and at least seven initial targets after the longest window."
            ),
        },
        "parameter_grid": {
            "windows": windows,
            "registered_alphas": registered_alphas,
            "pretest_lower_boundary_extension": LOWER_GRID_EXTENSION,
            "evaluated_alphas": alphas,
        },
        "grid_extension_amendment": {
            "status": "documented_before_registered_ridge_tuning_run",
            "trigger": (
                "An isolated base_train-only dry run selected the registered "
                "lower boundary alpha=0.0001."
            ),
            "rule": (
                "Add four smaller decade-spaced positive values and the natural "
                "alpha=0 ordinary-least-squares endpoint."
            ),
            "router_train_accessed_when_decided": False,
            "calibration_accessed_when_decided": False,
            "test_accessed_when_decided": False,
        },
        "selected_window": int(best["window"]),
        "selected_alpha": float(best["alpha"]),
        "selected_alpha_is_grid_boundary": bool(
            float(best["alpha"]) in {min(alphas), max(alphas)}
        ),
        "selected_alpha_is_natural_zero_endpoint": bool(
            float(best["alpha"]) == 0.0
        ),
        "unresolved_alpha_grid_boundary": False,
    ...
    for window in windows:
        part = tuning_summary.loc[tuning_summary["window"] == window].sort_values(
            "alpha"
        )
        axes[0].plot(
            part["alpha"],
    ...
    axes[0].set_xlabel("Ridge alpha (symmetric log scale; includes zero)")
    axes[0].set_ylabel("Equal-weight mean series RMSSE")
    ...
    bars = axes[1].bar(
        validation_counts["validation_count"].astype(str),
        validation_counts["size"],
        color="#59A14F",
    )
    axes[1].bar_label(bars)
    axes[1].set_title("Per-series validation block sizes")
    axes[1].set_xlabel("Days per fold")
    axes[1].set_ylabel("Series count")

    heatmap = (
        tuning_summary.pivot(index="window", columns="alpha", values="mean_RMSSE")
        .reindex(index=windows, columns=alphas)
    )
    image = axes[2].imshow(heatmap.to_numpy(), aspect="auto", cmap="viridis_r")
    axes[2].set_xticks(
        np.arange(len(alphas)), [f"{value:g}" for value in alphas], rotation=45
    )
    axes[2].set_yticks(np.arange(len(windows)), [str(value) for value in windows])
    axes[2].set_xlabel("Ridge alpha")
    axes[2].set_ylabel("Window (days)")
    axes[2].set_title("Mean RMSSE grid (lower is better)")
    fig.colorbar(image, ax=axes[2], shrink=0.82)
    axes[2].scatter(
        [alphas.index(float(best["alpha"]))],
        [windows.index(int(best["window"]))],
        marker="*",
        s=180,
        facecolor="white",
        edgecolor="black",
    )
    fig.suptitle("Weather Daily Ridge tuning within base_train", fontsize=15)
    ...
    report = {
        ...
        "alpha_grid_extended_after_base_only_boundary_check": True,
        "outputs": {
            "details": str(DETAIL_PATH),
            "summary": str(SUMMARY_PATH),
            "selected_parameters": str(SELECTED_PATH),
    ...
    print()
    print("Weather Daily Ridge 滚动调参全部通过")
    print("固定样本编号：", EXPECTED_SAMPLE_ID)
    print("调参数据范围：仅 base_train")
    print("滚动验证折数：", N_FOLDS)
    print("原始 alpha 数量：", len(registered_alphas))
    print("向下扩展后 alpha 数量：", len(alphas))
    print(
        "每条序列每折验证长度范围：",
        int(fold_manifest["validation_count"].min()),
        "至",
        int(fold_manifest["validation_count"].max()),
        "天",
    )
    ...
    print("最佳 alpha 是否为自然零端点：", "是" if float(best["alpha"]) == 0.0 else "否")
    print("是否仍有未解决的 alpha 网格边界：否")
    print("最佳平均 RMSSE：", f"{float(best['mean_RMSSE']):.6f}")
    print(
        "充分统计量与显式窗口核对最大差异：",
        f"{sufficient_statistic_max_difference:.3e}",
    )
    print(
        "与 sklearn Ridge 系数核对最大差异：",
        f"{coefficient_max_difference:.3e}",
    )
    print("折内零IQR回退记录数量：", int(fold_scalers["zero_iqr_fallback"].sum()))
