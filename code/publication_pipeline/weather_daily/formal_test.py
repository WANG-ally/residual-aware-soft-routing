#!/usr/bin/env python3
"""Freeze or execute the one-time Weather Daily evaluator.

The freeze mode verifies every final-pretest hash and reconstructs the complete
calibration segment. It never reads the test split. The execution mode is a
separate, one-time operation bound to the exact SHA-256 of this file."""
...
EXPECTED_FINAL_FREEZE_ID = (
    "004d19955e8b92b9b4fd3c9ee43732a75cf5d0a48bfe1bfd828bb13113c15cf1"
)
EXPECTED_FINAL_FREEZE_FILES = 90
EXPECTED_SERIES = 500
EXPECTED_PRETEST_ROWS = 1_842_491
EXPECTED_CALIBRATION_ROWS = 736_875
EXPECTED_CALIBRATION_STEPS = 5_867
EXPECTED_CALIBRATION_LENGTH_RANGE = (134, 5_867)
EXPECTED_TEST_ROWS = 1_105_774
EXPECTED_TEST_STEPS = 8_802
EXPECTED_TEST_LENGTH_RANGE = (200, 8_802)
SEASONAL_PERIOD = 7
LONG_CONTEXT = 28
MAX_RESIDUAL_LAG = 16
ROLLING_COVERAGE_DAYS = 56
SERIES_TYPES = ("maxtemp", "mintemp", "rain", "solar")

FINAL_LOCK_PATH = (
    PROJECT_ROOT / "results/weather_daily_final_pretest_lock_manifest.json"
)
AUTHORIZATION_PLAN_PATH = (
    PROJECT_ROOT / "results/weather_daily_final_test_authorization_plan.yaml"
)
...
PRETEST_PATH = PROJECT_ROOT / "results/weather_daily_pretest_predictions.parquet"
ROUTER_FEATURE_PATH = PROJECT_ROOT / "results/weather_daily_router_features.parquet"
CALIBRATION_ROUTER_PATH = (
    PROJECT_ROOT / "results/weather_daily_calibration_router_scores.parquet"
)
CALIBRATION_CONTROLLER_PATH = (
    PROJECT_ROOT / "results/weather_daily_calibration_controller_decisions.parquet"
)
BASELINE_CALIBRATION_PATH = (
    PROJECT_ROOT / "results/weather_daily_baseline_calibration_scores.parquet"
)
RIDGE_PARAM_PATH = PROJECT_ROOT / "results/weather_daily_selected_ridge_params.yaml"
LGBM_PARAM_PATH = (
    PROJECT_ROOT / "results/weather_daily_selected_lightgbm_params.yaml"
)
CONTROLLER_PATH = (
    PROJECT_ROOT / "results/weather_daily_selected_coverage_controller.yaml"
)
BASELINE_PARAM_PATH = (
    PROJECT_ROOT / "results/weather_daily_selected_baseline_params.yaml"
)
RIDGE_MODEL_PATH = PROJECT_ROOT / "models/weather_daily_ridge.joblib"
LGBM_MODEL_PATH = PROJECT_ROOT / "models/weather_daily_lightgbm.joblib"
FULL_ROUTER_MODEL_PATH = PROJECT_ROOT / "models/weather_daily_soft_router.joblib"

AUTHORIZATION_PATH = OUTPUT_ROOT / "logs/weather_daily_evaluator_authorization.json"
RECEIPT_PATH = OUTPUT_ROOT / "logs/weather_daily_formal_test_access_receipt.json"
PREDICTION_PATH = OUTPUT_ROOT / "results/weather_daily_test_predictions.parquet"
PER_SERIES_PATH = OUTPUT_ROOT / "results/weather_daily_test_per_series_metrics.csv"
AGGREGATE_PATH = OUTPUT_ROOT / "results/weather_daily_test_aggregate_metrics.csv"
COVERAGE_TRACE_PATH = OUTPUT_ROOT / "results/weather_daily_test_coverage_trace.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/weather_daily_test_method_comparison.png"
...
ALL_ROUTER_FEATURES = CONTEXT_FEATURES + [
    name
...
def maximum_absolute_difference(left: np.ndarray, right: np.ndarray) -> float:
    ...
    if left_array.shape != right_array.shape:
        raise ValueError(
            f"Array shape mismatch: {left_array.shape} != {right_array.shape}"
        )
    if left_array.size == 0:
        return 0.0
    if not np.isfinite(left_array).all() or not np.isfinite(right_array).all():
        raise ValueError("Nonfinite values occurred during calibration comparison")
...
def verify_final_lock() -> dict:
    ...
    if (
        ...
        or int(manifest.get("file_count", -1)) != EXPECTED_FINAL_FREEZE_FILES
...
def load_artifacts() -> dict:
    ...
    for method, metadata in baseline_params["methods"].items():
        ...
        if (
            bundle.get("dataset_id") != DATASET_ID
            or bundle.get("sample_id") != SAMPLE_ID
            or bundle.get("method") != method
            or bundle.get("test_accessed") is not False
            or bundle.get("calibration_used_for_model_fit") is not False
        ):
            raise ValueError(f"Baseline bundle identity/scope is invalid: {method}")
        baseline_bundles[method] = bundle

    if (
        plan.get("dataset_id") != DATASET_ID
        or plan.get("sample_id") != SAMPLE_ID
        or plan.get("formal_test_authorized") is not False
        or int(plan.get("formal_test_runs_completed", -1)) != 0
        or plan.get("status") != "all_pretest_models_frozen_evaluator_pending"
        or int(plan["test_partition"]["total_predictions"]) != EXPECTED_TEST_ROWS
    ...
    if (
        int(ridge_params["selected_window"]) != 56
        or float(ridge_params["selected_alpha"]) != 0.0
        or bool(ridge_params.get("unresolved_alpha_grid_boundary", True))
        or int(lgbm_params["selected_window"]) != 28
        or int(full_router["residual_lag"]) != 8
        or list(full_router["feature_names"]) != ALL_ROUTER_FEATURES[:30]
        or len(full_router["feature_names"]) != 30
        or float(controller["primary_target_simple_coverage"]) != 0.7
        or float(controller["new_segment_initialization"]["bias"]) != 0.0
        or controller["new_segment_initialization"][
            "calibration_final_bias_is_not_carried_to_test"
        ]
        is not True
        or int(controller["rolling_diagnostic_window_days"])
        != ROLLING_COVERAGE_DAYS
    ):
        raise ValueError("Frozen Weather Daily parameters differ from the test plan")
    ...
    if (
        set(baseline_params["methods"]) != expected_baselines
        or set(baseline_bundles) != expected_baselines - {"hard_aalf_like_router"}
    ):
        raise ValueError("The six frozen baselines are incomplete")

    scalers = pd.read_csv(SCALER_PATH).sort_values("sample_order", kind="stable")
    split_manifest = pd.read_csv(SPLIT_MANIFEST_PATH).sort_values(
        "sample_order", kind="stable"
    )
    pretest = pd.read_parquet(PRETEST_PATH)
    if (
        len(scalers) != EXPECTED_SERIES
        or len(split_manifest) != EXPECTED_SERIES
        or scalers["source_split"].ne("base_train").any()
        or set(scalers["sample_id"].astype(str)) != {SAMPLE_ID}
        or set(split_manifest["sample_id"].astype(str)) != {SAMPLE_ID}
        or set(split_manifest["series_type"].astype(str)) != set(SERIES_TYPES)
        or set(pretest["split"].astype(str)) != {"router_train", "calibration"}
        or set(pretest["sample_id"].astype(str)) != {SAMPLE_ID}
        or len(pretest) != EXPECTED_PRETEST_ROWS
    ):
        raise ValueError("Scaler, split manifest, or pretest-history scope is invalid")
    if (
        int(split_manifest["calibration_count"].min())
        != EXPECTED_CALIBRATION_LENGTH_RANGE[0]
        or int(split_manifest["calibration_count"].max())
        != EXPECTED_CALIBRATION_LENGTH_RANGE[1]
        or int(split_manifest["calibration_count"].sum())
        != EXPECTED_CALIBRATION_ROWS
        or int(split_manifest["test_count"].min())
        != EXPECTED_TEST_LENGTH_RANGE[0]
        or int(split_manifest["test_count"].max())
        != EXPECTED_TEST_LENGTH_RANGE[1]
        or int(split_manifest["test_count"].sum()) != EXPECTED_TEST_ROWS
    ):
        raise ValueError("Registered variable-length segment dimensions changed")
...
def build_segment(
    ...
    """Build frozen causal predictions and routing features for one segment."""

    if segment_name not in {"calibration", "test"}:
        raise ValueError(f"Unsupported segment: {segment_name}")
    frames = {
        str(series_id): group.sort_values("time_index", kind="stable").reset_index(
            drop=True
        )
        for series_id, group in raw.groupby("series_id", sort=False, observed=True)
    }
    if len(frames) != EXPECTED_SERIES:
        raise ValueError(f"Expected {EXPECTED_SERIES} series, observed {len(frames)}")

    manifest_lookup = {
        str(row.series_id): row
        for row in artifacts["split_manifest"].itertuples(index=False)
    }
    scaler_lookup = {
        str(row.series_id): (float(row.median), float(row.scale_used))
        for row in artifacts["scalers"].itertuples(index=False)
    }
    if set(frames) != set(manifest_lookup) or set(frames) != set(scaler_lookup):
        raise ValueError("Raw data, manifest, and scaler series IDs differ")
    ...
    for series_id, frame in frames.items():
        manifest_row = manifest_lookup[series_id]
        ...
        observed_type = frame["series_type"].astype(str).unique().tolist()
        if observed_type != [str(manifest_row.series_type)]:
            raise ValueError(f"{series_id} has an inconsistent series type")
        ...
        expected_count = int(getattr(manifest_row, f"{segment_name}_count"))
        expected_start = int(getattr(manifest_row, f"{segment_name}_start"))
        expected_end = int(getattr(manifest_row, f"{segment_name}_end"))
        if (
            len(targets) != expected_count
            or int(targets[0]) != expected_start
            or int(targets[-1]) != expected_end
            or not np.all(np.diff(targets) == 1)
        ):
        ...
        lgbm_prediction = lgbm_model.booster_.predict(lgbm_x) * scale + median
        ...
        part = frame.loc[
            segment_mask,
            ["dataset_id", "series_id", "series_type", "time_index", "split"],
        ].copy()
        part.insert(1, "sample_id", SAMPLE_ID)
        part["series_id"] = part["series_id"].astype(str)
        part["series_type"] = part["series_type"].astype(str)
        ...
        part["mean_7_scaled"] = ((mean_7 - median) / scale).astype(np.float32)
        part["trend_7_scaled"] = (
            (values[targets - 1] - values[targets - 1 - SEASONAL_PERIOD]) / scale
        ).astype(np.float32)
        part["volatility_7_scaled"] = (std_7 / scale).astype(np.float32)
        part["trend_28_scaled"] = (
            (values[targets - 1] - values[targets - 1 - LONG_CONTEXT]) / scale
        ).astype(np.float32)
        part["volatility_28_scaled"] = (std_28 / scale).astype(np.float32)
        ...
        for series_type in SERIES_TYPES:
            part[f"series_type_{series_type}"] = np.float32(
                str(manifest_row.series_type) == series_type
            )
        ...
        for lag in range(1, MAX_RESIDUAL_LAG + 1):
            ...
            if not np.isfinite(ridge_lag).all() or not np.isfinite(lgbm_lag).all():
                raise ValueError(f"{series_id} lacks residual history for lag {lag}")
    ...
    if (
        ...
        or segment["sample_id"].ne(SAMPLE_ID).any()
        or set(segment["split"].astype(str)) != {segment_name}
        or segment["series_id"].nunique() != EXPECTED_SERIES
        or set(segment["series_type"].astype(str)) != set(SERIES_TYPES)
        or not np.isfinite(
            segment[ALL_ROUTER_FEATURES].to_numpy(dtype=np.float64)
        ).all()
    ):
...
def apply_methods(frame: pd.DataFrame, artifacts: dict) -> pd.DataFrame:
    ...
    # The frozen local-weighting stage deliberately used float32 router inference; preserving this
    # path is necessary for bit-level calibration reconstruction.
    full_x = full_bundle["scaler"].transform(
        frame[full_bundle["feature_names"]].to_numpy(dtype=np.float32)
    ).astype(np.float32)
    ...
    ridge_squared_error = (frame["y_true"] - frame["ridge_prediction"]) ** 2
    lgbm_squared_error = (frame["y_true"] - frame["lightgbm_prediction"]) ** 2
...
def expected_calibration_differences(computed: pd.DataFrame) -> dict[str, float]:
    ...
    for expected in sorted_frames[1:]:
        ...
        if set(expected["sample_id"].astype(str)) != {SAMPLE_ID}:
            raise ValueError("Frozen calibration evidence has a different sample ID")
    ...
    differences = {
        ...
        "router_features": maximum_absolute_difference(
            calculated[ALL_ROUTER_FEATURES].to_numpy(dtype=np.float64),
            expected_features[ALL_ROUTER_FEATURES].to_numpy(dtype=np.float64),
        ),
    ...
    calculated_decision = calculated[
        "use_ridge_adaptive_full_router"
    ].to_numpy(dtype=bool)
    expected_decision = expected_controller["use_ridge"].to_numpy(dtype=bool)
    differences["controller_decision_mismatch_count"] = float(
        np.sum(calculated_decision != expected_decision)
    )
    calculated_step = calculated["relative_segment_step"].to_numpy(
        dtype=np.int64
    )
    expected_step = expected_controller["relative_calibration_step"].to_numpy(
        dtype=np.int64
    )
    differences["relative_step_mismatch_count"] = float(
        np.sum(calculated_step != expected_step)
    )
    return differences
...
def dry_run_calibration(artifacts: dict) -> dict:
    ...
    per_series_lengths = computed.groupby("series_id", observed=True).size()
    active_sizes = computed.groupby("relative_segment_step", observed=True).size()
    if (
        ...
        or int(per_series_lengths.min()) != EXPECTED_CALIBRATION_LENGTH_RANGE[0]
        or int(per_series_lengths.max()) != EXPECTED_CALIBRATION_LENGTH_RANGE[1]
    ...
    return {
        ...
        "minimum_active_series": int(active_sizes.min()),
        "maximum_active_series": int(active_sizes.max()),
        "minimum_series_length": int(per_series_lengths.min()),
        "maximum_series_length": int(per_series_lengths.max()),
        "tolerance": tolerance,
...
def compute_metrics(
    ...
    coverage_trace["rolling_56d_simple_count"] = coverage_trace.groupby(
        "method", sort=False, observed=True
    )["simple_count"].transform(
        lambda values: values.rolling(
            ROLLING_COVERAGE_DAYS, min_periods=ROLLING_COVERAGE_DAYS
        ).sum()
    )
    coverage_trace["rolling_56d_rows"] = coverage_trace.groupby(
        "method", sort=False, observed=True
    ...
    coverage_trace["rolling_56d_simple_coverage"] = (
        coverage_trace["rolling_56d_simple_count"]
        / coverage_trace["rolling_56d_rows"]
    )
    ...
    for method in METHODS:
        ...
        rolling = method_trace["rolling_56d_simple_coverage"].dropna()
        worst_rolling = (
        ...
        aggregate_records.append(
            {
                ...
                "worst_56d_weighted_coverage_violation": worst_rolling,
...
def save_figure(
    ...
    axes[0].set_title("Weather Daily formal-test accuracy")
    ...
    for method, color in (
        ...
        axes[2].plot(
            ...
            part["rolling_56d_simple_coverage"],
            label=method,
            color=color,
        )
    axes[2].axhline(target_coverage, color="black", linestyle="--")
    axes[2].set_ylim(0.0, 1.0)
    axes[2].set_xlabel("Days since series entered test")
    axes[2].set_ylabel("Weighted rolling 56-day Ridge coverage")
...
def freeze_evaluator() -> None:
    ...
    print()
    print("Weather Daily 正式测试执行器冻结与校准干运行全部通过")
    ...
    print(
        "活动序列数量范围：",
        f"{dry_run['minimum_active_series']} 至 {dry_run['maximum_active_series']}",
    )
    print(
        "校准重算最大差异：",
...
def execute_formal_test() -> None:
    ...
    evaluator_hash = sha256_file(EVALUATOR_PATH)
    if (
        authorization.get("dataset_id") != DATASET_ID
        or authorization.get("sample_id") != SAMPLE_ID
    ...
    try:
        ...
        per_series_lengths = test_frame.groupby("series_id", observed=True).size()
        active_sizes = test_frame.groupby(
            "relative_segment_step", observed=True
        ).size()
        manifest_lengths = artifacts["split_manifest"].set_index("series_id")[
            "test_count"
        ].sort_index()
        observed_lengths = per_series_lengths.copy()
        observed_lengths.index = observed_lengths.index.astype(str)
        observed_lengths = observed_lengths.sort_index()
        manifest_lengths.index = manifest_lengths.index.astype(str)
        manifest_lengths = manifest_lengths.sort_index()
        if (
            len(test_frame) != EXPECTED_TEST_ROWS
            or test_frame["series_id"].nunique() != EXPECTED_SERIES
            or test_frame["relative_segment_step"].nunique() != EXPECTED_TEST_STEPS
            or int(per_series_lengths.min()) != EXPECTED_TEST_LENGTH_RANGE[0]
            or int(per_series_lengths.max()) != EXPECTED_TEST_LENGTH_RANGE[1]
            or int(active_sizes.min()) != 1
            or int(active_sizes.max()) != EXPECTED_SERIES
            or not observed_lengths.equals(manifest_lengths)
        ...
        if (
            ...
            or set(per_series["sample_id"].astype(str)) != {SAMPLE_ID}
        ):
    ...
    print()
    print("Weather Daily 唯一一次正式测试全部完成")
    print("固定样本编号：", SAMPLE_ID)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze or execute the one-time Weather Daily formal evaluator"
    )
