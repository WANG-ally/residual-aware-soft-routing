#!/usr/bin/env python3
"""Statistical analysis from saved Pedestrian formal metrics only.
...
Pedestrian series, so variable-length series receive equal statistical weight."""
...
EXPECTED_EVALUATOR_HASH = (
    "417f7583893db34d998bd27ce7af9703bb4e3add996e338f16575d77d3f6416f"
)
EXPECTED_AUTHORIZATION_ID = (
    "e8266a0ab0939f2f830384a239a12073f7060c35867bd26c9661341c6d28ffc0"
)
EXPECTED_FINAL_FREEZE_ID = (
    "786a38ac96473411d414faac48f32af7f0d690b3915cc266349bf0a0eb86ee07"
)

RECEIPT_PATH = PROJECT_ROOT / "logs/pedestrian_formal_test_access_receipt.json"
AUTHORIZATION_PATH = PROJECT_ROOT / "logs/pedestrian_evaluator_authorization.json"
EVALUATOR_PATH = PROJECT_ROOT / "code/publication_pipeline/pedestrian/formal_test.py"
PER_SERIES_PATH = (
    PROJECT_ROOT / "results/pedestrian_test_per_series_metrics.csv"
)
...
CI_PATH = OUTPUT_ROOT / "results/pedestrian_method_bootstrap_ci.csv"
PAIRWISE_PATH = OUTPUT_ROOT / "results/pedestrian_primary_pairwise_tests.csv"
NONINFERIORITY_PATH = (
    OUTPUT_ROOT / "results/pedestrian_noninferiority_tests.csv"
)
SUMMARY_PATH = OUTPUT_ROOT / "results/pedestrian_statistical_summary.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/pedestrian_statistical_analysis_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/pedestrian_statistical_comparison.png"
REPORT_PATH = OUTPUT_ROOT / "logs/pedestrian_statistical_analysis_report.json"
...
def main() -> None:
    ...
    formal_identity_valid = bool(
        ...
        and int(receipt.get("formal_test_rows", -1)) == 469_884
    ...
    per_series_identity_valid = bool(
        ...
        .eq(1)
        .all()
        and int(per_series["test_rows"].min()) == 87
        and int(per_series["test_rows"].max()) == 14_464
    )
    record_check(
    ...
    pairwise["direction"] = np.where(
        pairwise["mean_difference_primary_minus_comparator"] < 0.0,
        "primary_better",
        "comparator_better",
    )
    ...
    significantly_better = pairwise.loc[
        pairwise["significant_after_holm"]
        & pairwise["direction"].eq("primary_better"),
        "comparator",
    ].tolist()
    significantly_worse = pairwise.loc[
        pairwise["significant_after_holm"]
        & pairwise["direction"].eq("comparator_better"),
        "comparator",
    ].tolist()
    ...
    if not failed_checks.empty:
        raise AssertionError(
            f"Pedestrian statistical checks failed: {failed_checks.to_dict('records')}"
        )
