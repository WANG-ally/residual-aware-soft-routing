#!/usr/bin/env python3
"""Base-only scaling and lazy causal windows for Pedestrian."""
...
SPLIT_CHECKS_PATH = PROJECT_ROOT / "results/pedestrian_split_checks.csv"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/pedestrian_split_summary.yaml"

SCALER_PATH = OUTPUT_ROOT / "results/pedestrian_scaler_parameters.csv"
COUNT_PATH = OUTPUT_ROOT / "results/pedestrian_window_candidate_counts.csv"
SERIES_COUNT_PATH = OUTPUT_ROOT / "results/pedestrian_window_counts_by_series.csv"
PREVIEW_PATH = OUTPUT_ROOT / "results/pedestrian_window_preview.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/pedestrian_window_preparation_checks.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/pedestrian_window_preparation_summary.yaml"
FIGURE_PATH = OUTPUT_ROOT / "figures/pedestrian_window_example.png"
REPORT_PATH = OUTPUT_ROOT / "logs/pedestrian_window_preparation_report.json"
...
def main() -> None:
    ...
    if candidate_windows != [24, 48, 168]:
        raise AssertionError(
            f"Hourly candidate windows must be [24, 48, 168], got {candidate_windows}"
        )
    ...
    data = data.sort_values(["series_id", "time_index"], kind="stable").reset_index(drop=True)
    ...
    for series_id_raw, group in data.groupby("series_id", sort=False, observed=True):
        ...
        contiguous_index_ok = contiguous_index_ok and bool(
            np.array_equal(indices, np.arange(len(group), dtype=np.int64))
        )
        hourly_timestamp_ok = hourly_timestamp_ok and bool(
        ...
        base_values = base["value"].to_numpy(dtype=float)
    ...
    for row in scalers.itertuples(index=False):
        ...
        scaler_manifest_match = scaler_manifest_match and bool(
        ...
        for window in candidate_windows:
            ...
            for split_name in SPLIT_NAMES:
                ...
                if count > 0:
                    ...
                    causal_boundary_ok = causal_boundary_ok and bool(
                        first_history_start >= 0
                        and first_history_end < first_target
                        and last_history_end < last_target
                    )
            record["total"] = total_count
            count_formula_ok = count_formula_ok and bool(
                total_count == total_length - window
            )
    ...
    # Preview five concrete causal samples from the shortest series.
    ...
    raw_values = example["value"].to_numpy(dtype=float)
    ...
    for target_position in range(preview_window, preview_window + 5):
        record: dict[str, object] = {
            ...
            "history_start_index": int(target_position - preview_window),
            "history_end_index": int(target_position - 1),
            "target_time_index": int(target_position),
    ...
    expected_totals = {
        window: int(len(data) - len(manifest) * window) for window in candidate_windows
    }
    observed_totals = {
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "candidate_windows_match_registration",
            candidate_windows == [24, 48, 168],
            str(candidate_windows),
        ),
        (
            "one_scaler_per_series",
        ...
        (
            "time_indices_are_contiguous",
            contiguous_index_ok,
            str(contiguous_index_ok),
        ),
        (
            "timestamps_are_hourly",
            hourly_timestamp_ok,
            str(hourly_timestamp_ok),
        ),
        (
            "every_window_history_precedes_target",
            causal_boundary_ok,
        ...
        (
            "aggregate_window_counts_match_expected",
        ...
        (
            ...
            bool(
                (
                    preview["history_end_index"]
                    < preview["target_time_index"]
                ).all()
            ),
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Pedestrian window preparation failed: {message}")
    ...
    summary = {
        ...
        "window_storage": {
            "strategy": "lazy/on-demand construction from the processed long table",
            ...
            "reason": (
                "Avoid multi-gigabyte duplicate matrices and construct only the "
                "training fold currently allowed by the chronology."
            ),
        },
    ...
    axes[0, 0].set_title("Base-train scaler parameters across series")
    axes[0, 0].set_xlabel("Base-train median")
    ...
    axes[0, 1].set_title("Available causal samples by window")
    axes[0, 1].set_xlabel("Window length (hours)")
    ...
    example_scaled = scale_values(raw_values, median, scale)
    for split_name in SPLIT_NAMES:
        mask = example_splits[history_indices] == split_name
        if mask.any():
            axes[1, 1].scatter(
                history_indices[mask],
                example_scaled[history_indices[mask]],
                color=COLORS[split_name],
                s=10,
    ...
    axes[1, 1].set_title(
        f"{preview_series}: 168-hour history before the first test target"
    )
    axes[1, 1].set_xlabel("Time index (hour)")
    axes[1, 1].set_ylabel("Base-train robust-scaled count")
    axes[1, 1].grid(alpha=0.2)
    axes[1, 1].legend(frameon=False, fontsize=8)

    fig.suptitle(
        "Pedestrian hourly: base-only scaling and causal window preparation",
        fontsize=15,
    )
