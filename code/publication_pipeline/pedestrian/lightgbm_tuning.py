#!/usr/bin/env python3
"""Sampled base-only rolling tuning for Pedestrian LightGBM."""
...
RIDGE_FOLD_PATH = PROJECT_ROOT / "results/pedestrian_ridge_fold_manifest.csv"
RIDGE_FOLD_SCALER_PATH = PROJECT_ROOT / "results/pedestrian_ridge_fold_scalers.csv"
RIDGE_CHECKS_PATH = PROJECT_ROOT / "results/pedestrian_ridge_tuning_checks.csv"

DETAIL_PATH = OUTPUT_ROOT / "results/pedestrian_lightgbm_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/pedestrian_lightgbm_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/pedestrian_selected_lightgbm_params.yaml"
SAMPLING_MANIFEST_PATH = OUTPUT_ROOT / "results/pedestrian_lightgbm_sampling_manifest.csv"
SAMPLING_POSITIONS_PATH = OUTPUT_ROOT / "results/pedestrian_lightgbm_sample_positions.parquet"
CHECKS_PATH = OUTPUT_ROOT / "results/pedestrian_lightgbm_tuning_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/pedestrian_lightgbm_tuning.png"
REPORT_PATH = OUTPUT_ROOT / "logs/pedestrian_lightgbm_tuning_report.json"
...
def systematic_sample_by_series(
    ...
    counts = np.asarray([available_counts[series_id] for series_id in series_order], dtype=np.int64)
    if np.any(counts <= 0):
    ...
    global_positions = np.floor(
        (np.arange(sample_size, dtype=np.float64) + 0.5) * total / sample_size
    ).astype(np.int64)
...
def main() -> None:
    ...
    for series_id, values in series_values.items():
        for fold in range(1, N_FOLDS + 1):
            ...
            shared_fold_artifacts_ok = shared_fold_artifacts_ok and bool(
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        for window in windows:
            available_counts = {
                series_id: int(fold_lookup.loc[(series_id, fold), "train_count"]) - window
            ...
            for series_id in series_order:
                ...
                sampled_count = int(len(targets))
                all_series_represented = all_series_represented and sampled_count > 0
                all_samples_causal = all_samples_causal and bool(
                    sampled_count > 0
                ...
                train_y_parts.append(scaled_values[targets].astype(np.float32, copy=True))
            ...
            if len(train_x) != expected_train_samples or sample_order_cursor != expected_train_samples:
                raise AssertionError("Sampled training row count mismatch")
            full_validation_used = full_validation_used and bool(
                len(validation_x) == expected_validation_samples
            )
                ...
                records.append(
                    {
                        ...
                        "training_sampling_fraction": float(len(train_y) / total_available),
    ...
    expected_sample_position_rows = (
        N_FOLDS * len(windows) * MAX_TUNING_TRAIN_TARGETS
    )
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "ridge_fold_boundaries_and_scalers_reused",
            shared_fold_artifacts_ok,
            str(shared_fold_artifacts_ok),
        ),
        (
            "parameter_grid_has_16_configurations",
        ...
        (
            "every_series_is_represented_in_every_sample",
            all_series_represented,
            str(all_series_represented),
        ),
        (
            "exact_sample_cap_used_for_each_fold_window",
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Pedestrian LightGBM tuning failed: {message}")
    ...
    selected = {
        ...
        "fold_boundaries": "identical to pedestrian_ridge_fold_manifest.csv",
        "training_sample": {
    ...
    axes[0].set_xlabel("Mean series RMSSE (lower is better)")
    ...
    axes[1].set_ylabel("Mean series RMSSE")
    axes[1].grid(axis="y", alpha=0.2)
    axes[1].legend(frameon=False)

    fig.suptitle(
        "Pedestrian LightGBM tuning within base_train only",
        fontsize=15,
    )
    ...
    print()
    print("Pedestrian LightGBM 滚动调参全部通过")
