#!/usr/bin/env python3
"""Calibrate the Pedestrian adaptive coverage controller.

Only the saved calibration routing scores are read.  No model is loaded or
refitted, and the test split is never accessed.  Because Pedestrian series have
different lengths, online batches are aligned by hours elapsed since each
series entered its calibration segment."""
...
SCORE_PATH = PROJECT_ROOT / "results/pedestrian_calibration_router_scores.parquet"
LOCAL_PARAMETER_PATH = (
    PROJECT_ROOT / "results/pedestrian_selected_local_weighting_params.yaml"
)
LOCAL_CHECKS_PATH = PROJECT_ROOT / "results/pedestrian_local_weighting_checks.csv"

TUNING_PATH = OUTPUT_ROOT / "results/pedestrian_coverage_controller_tuning.csv"
TRACE_PATH = OUTPUT_ROOT / "results/pedestrian_calibration_controller_trace.csv"
DECISION_PATH = (
    OUTPUT_ROOT / "results/pedestrian_calibration_controller_decisions.parquet"
)
THRESHOLD_PATH = OUTPUT_ROOT / "results/pedestrian_calibration_thresholds.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/pedestrian_selected_coverage_controller.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/pedestrian_coverage_calibration_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/pedestrian_coverage_calibration.png"
REPORT_PATH = OUTPUT_ROOT / "logs/pedestrian_coverage_calibration_report.json"

DATASET_ID = "pedestrian_hourly"
EXPECTED_ROWS = 313_219
EXPECTED_SERIES = 66
EXPECTED_RELATIVE_STEPS = 9_642
...
def simulate_controller(
    ...
    trace["rolling_168h_simple_coverage"] = rolling_simple_count / rolling_rows
    valid_rolling = trace["rolling_168h_simple_coverage"].dropna()
    ...
    summary: dict[str, object] = {
        ...
        "worst_168h_weighted_coverage_violation": worst_rolling_violation,
...
def main() -> None:
    ...
    if feasible.empty:
        ...
        ranked = tuning.sort_values(
            [
                ...
                "worst_168h_weighted_coverage_violation",
                "eta",
            ],
            kind="stable",
        )
    ...
    checks_items: list[tuple[str, bool, str]] = [
        ...
        (
            ...
            int(per_series_lengths.min()) == 57
            and int(per_series_lengths.max()) == 9642,
            (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Pedestrian coverage calibration failed: {message}")
    ...
    selected_parameters = {
        ...
        "calibration_worst_168h_weighted_coverage_violation": float(
            selected_summary["worst_168h_weighted_coverage_violation"]
        ),
    ...
    axes[1].plot(
        selected_trace["relative_calibration_step"],
        selected_trace["rolling_168h_simple_coverage"],
        color="#E15759",
        linewidth=1.6,
        label="Weighted 168-hour coverage",
    )
    axes[1].axhline(
        primary_coverage,
        color="black",
    ...
    figure.suptitle(
        "Pedestrian calibration-only coverage-controller selection", fontsize=15
    )
    ...
    report = {
        ...
        "selected_worst_168h_violation": float(
            selected_summary["worst_168h_weighted_coverage_violation"]
        ),
    ...
    print(
        "最差168小时加权覆盖率偏差：",
        f"{float(selected_summary['worst_168h_weighted_coverage_violation']):.6f}",
    )
