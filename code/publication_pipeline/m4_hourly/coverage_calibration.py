#!/usr/bin/env python3
"""Calibrate the M4 Hourly adaptive coverage controller.

Only the saved calibration routing scores are read.  No model is loaded or
refitted, and the test split is never accessed.  Because M4 Hourly series have
...
SCORE_PATH = PROJECT_ROOT / "results/m4_hourly_calibration_router_scores.parquet"
LOCAL_PARAMETER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_local_weighting_params.yaml"
)
LOCAL_CHECKS_PATH = PROJECT_ROOT / "results/m4_hourly_local_weighting_checks.csv"

TUNING_PATH = OUTPUT_ROOT / "results/m4_hourly_coverage_controller_tuning.csv"
TRACE_PATH = OUTPUT_ROOT / "results/m4_hourly_calibration_controller_trace.csv"
DECISION_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_calibration_controller_decisions.parquet"
)
THRESHOLD_PATH = OUTPUT_ROOT / "results/m4_hourly_calibration_thresholds.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/m4_hourly_selected_coverage_controller.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_coverage_calibration_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_coverage_calibration.png"
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_coverage_calibration_report.json"

DATASET_ID = "m4_hourly"
EXPECTED_ROWS = 37_006
EXPECTED_SERIES = 414
EXPECTED_RELATIVE_STEPS = 100
...
def main() -> None:
    ...
    checks_items: list[tuple[str, bool, str]] = [
        ...
        (
            ...
            int(per_series_lengths.min()) == 74
            and int(per_series_lengths.max()) == 100,
            (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly coverage calibration failed: {message}")
    ...
    figure.suptitle(
        "M4 Hourly calibration-only coverage-controller selection", fontsize=15
    )
