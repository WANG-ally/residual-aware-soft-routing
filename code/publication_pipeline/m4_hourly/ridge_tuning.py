#!/usr/bin/env python3
"""Base-train-only rolling validation for M4 Hourly Ridge AR."""
...
WINDOW_SUMMARY_PATH = PROJECT_ROOT / "results/m4_hourly_window_preparation_summary.yaml"
WINDOW_CHECKS_PATH = PROJECT_ROOT / "results/m4_hourly_window_preparation_checks.csv"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/m4_hourly_split_summary.yaml"

DETAIL_PATH = OUTPUT_ROOT / "results/m4_hourly_ridge_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/m4_hourly_ridge_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/m4_hourly_selected_ridge_params.yaml"
FOLD_PATH = OUTPUT_ROOT / "results/m4_hourly_ridge_fold_manifest.csv"
FOLD_SCALER_PATH = OUTPUT_ROOT / "results/m4_hourly_ridge_fold_scalers.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_ridge_tuning_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_ridge_tuning.png"
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_ridge_tuning_report.json"
...
def main() -> None:
    ...
    if len(alphas) != 9 or len(set(alphas)) != 9 or min(alphas) <= 0.0:
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "all_414_series_are_present",
            len(series_values) == int(split_summary["series_count"]),
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly Ridge tuning failed: {message}")
    ...
    fig.suptitle("M4 Hourly Ridge tuning within base_train", fontsize=15)
    ...
    print()
    print("M4 Hourly Ridge 滚动调参全部通过")
