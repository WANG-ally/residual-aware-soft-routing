#!/usr/bin/env python3
"""Local-weighting ablation and final M4 Hourly soft-router fit.
...
FEATURE_CHECKS_PATH = PROJECT_ROOT / "results/m4_hourly_router_feature_checks.csv"
SELECTED_ROUTER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_soft_router_params.yaml"
)
ROUTER_DETAIL_PATH = (
    PROJECT_ROOT / "results/m4_hourly_soft_router_rolling_validation.csv"
)
ROUTER_FOLD_PATH = PROJECT_ROOT / "results/m4_hourly_soft_router_fold_manifest.csv"
ROUTER_SAMPLE_PATH = (
    PROJECT_ROOT / "results/m4_hourly_soft_router_sample_positions.parquet"
)
ROUTER_CHECKS_PATH = (
    PROJECT_ROOT / "results/m4_hourly_soft_router_tuning_checks.csv"
)
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"

DETAIL_PATH = OUTPUT_ROOT / "results/m4_hourly_local_weighting_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/m4_hourly_local_weighting_tuning_summary.csv"
SELECTED_LOCAL_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_selected_local_weighting_params.yaml"
)
FINAL_SAMPLE_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_final_router_sample_positions.parquet"
)
NEIGHBOR_MANIFEST_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_local_weighting_neighbor_manifest.csv"
)
COEFFICIENT_PATH = OUTPUT_ROOT / "results/m4_hourly_router_coefficients.csv"
CALIBRATION_SCORE_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_calibration_router_scores.parquet"
)
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_local_weighting_checks.csv"
MODEL_PATH = OUTPUT_ROOT / "models/m4_hourly_soft_router.joblib"
FIGURE_PATH = (
    OUTPUT_ROOT / "figures/m4_hourly_router_coefficients_and_scores.png"
)
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_local_weighting_report.json"
...
def main() -> None:
    ...
    if not upstream_checks_passed:
        raise AssertionError("An upstream M4 Hourly audit did not pass")
    ...
    if len(series_order) != 414:
        raise AssertionError(f"Expected 414 series, found {len(series_order)}")
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly local weighting failed: {message}")
    ...
    axes[0].set_title("Final M4 Hourly soft-router coefficients")
    ...
    figure.suptitle(
        "M4 Hourly final soft router: coefficients and calibration scores",
        fontsize=14,
    )
    ...
    print()
    print("M4 Hourly 局部加权与最终路由器训练全部通过")
