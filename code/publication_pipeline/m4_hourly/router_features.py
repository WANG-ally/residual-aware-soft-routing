#!/usr/bin/env python3
"""Causal context and past-residual features for M4 Hourly routing."""
...
PREDICTION_PATH = PROJECT_ROOT / "results/m4_hourly_pretest_predictions.parquet"
SCALER_PATH = PROJECT_ROOT / "results/m4_hourly_scaler_parameters.csv"
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"
BASE_CHECKS_PATH = PROJECT_ROOT / "results/m4_hourly_base_model_fit_checks.csv"
BASE_METADATA_PATH = PROJECT_ROOT / "results/m4_hourly_base_model_fit_metadata.yaml"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/m4_hourly_split_summary.yaml"

FEATURE_PATH = OUTPUT_ROOT / "results/m4_hourly_router_features.parquet"
MANIFEST_PATH = OUTPUT_ROOT / "results/m4_hourly_router_feature_manifest.csv"
COUNT_PATH = OUTPUT_ROOT / "results/m4_hourly_router_feature_counts.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_router_feature_checks.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/m4_hourly_router_feature_summary.yaml"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_router_target_distribution.png"
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_router_feature_report.json"
...
def main() -> None:
    ...
    all_features_finite = bool(
        np.isfinite(features[feature_names].to_numpy(dtype=np.float64)).all()
    )
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        ("all_414_series_are_present", len(raw_series) == EXPECTED_SERIES, str(len(raw_series))),
        ...
        (
            ...
            len(manifest) == 46 and len(feature_names) == 46,
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly router-feature audit failed: {message}")
    ...
    fig.suptitle("M4 Hourly causal routing features and targets", fontsize=15)
