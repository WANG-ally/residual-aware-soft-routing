#!/usr/bin/env python3
"""Freeze or execute the one-time M4 Hourly formal evaluator.
...
EXPECTED_SERIES = 414
EXPECTED_CALIBRATION_ROWS = 37_006
EXPECTED_CALIBRATION_STEPS = 100
EXPECTED_TEST_ROWS = 56_337
EXPECTED_TEST_STEPS = 152
SEASONAL_PERIOD = 24
LONG_CONTEXT = 168
MAX_RESIDUAL_LAG = 16
...
FINAL_LOCK_PATH = (
    PROJECT_ROOT / "results/m4_hourly_final_pretest_lock_manifest.json"
)
AUTHORIZATION_PLAN_PATH = (
    PROJECT_ROOT / "results/m4_hourly_final_test_authorization_plan.yaml"
)
...
SCALER_PATH = PROJECT_ROOT / "results/m4_hourly_scaler_parameters.csv"
PRETEST_PATH = PROJECT_ROOT / "results/m4_hourly_pretest_predictions.parquet"
ROUTER_FEATURE_PATH = PROJECT_ROOT / "results/m4_hourly_router_features.parquet"
CALIBRATION_ROUTER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_calibration_router_scores.parquet"
)
CALIBRATION_CONTROLLER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_calibration_controller_decisions.parquet"
)
BASELINE_CALIBRATION_PATH = (
    PROJECT_ROOT / "results/m4_hourly_baseline_calibration_scores.parquet"
)
RIDGE_PARAM_PATH = PROJECT_ROOT / "results/m4_hourly_selected_ridge_params.yaml"
LGBM_PARAM_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_lightgbm_params.yaml"
)
CONTROLLER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_coverage_controller.yaml"
)
BASELINE_PARAM_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_baseline_params.yaml"
)
RIDGE_MODEL_PATH = PROJECT_ROOT / "models/m4_hourly_ridge.joblib"
LGBM_MODEL_PATH = PROJECT_ROOT / "models/m4_hourly_lightgbm.joblib"
FULL_ROUTER_MODEL_PATH = PROJECT_ROOT / "models/m4_hourly_soft_router.joblib"

AUTHORIZATION_PATH = OUTPUT_ROOT / "logs/m4_hourly_evaluator_authorization.json"
RECEIPT_PATH = OUTPUT_ROOT / "logs/m4_hourly_formal_test_access_receipt.json"
PREDICTION_PATH = OUTPUT_ROOT / "results/m4_hourly_test_predictions.parquet"
PER_SERIES_PATH = OUTPUT_ROOT / "results/m4_hourly_test_per_series_metrics.csv"
AGGREGATE_PATH = OUTPUT_ROOT / "results/m4_hourly_test_aggregate_metrics.csv"
COVERAGE_TRACE_PATH = OUTPUT_ROOT / "results/m4_hourly_test_coverage_trace.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_test_method_comparison.png"
...
def load_artifacts() -> dict:
    ...
    if (
        ...
        raise ValueError("Frozen M4 Hourly parameters differ from the test plan")
...
def build_segment(
    ...
    if len(frames) != EXPECTED_SERIES:
        raise ValueError(f"Expected 414 series, observed {len(frames)}")
...
def save_figure(
    ...
    axes[0].set_title("M4 Hourly formal-test accuracy")
...
def freeze_evaluator() -> None:
    ...
    print()
    print("M4 Hourly 正式测试执行器冻结与校准干运行全部通过")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze or execute the one-time M4 Hourly formal evaluator"
    )
