#!/usr/bin/env python3
"""Leakage-safe four-stage split for M4 Hourly.
...
AUDIT_SUMMARY_PATH = PROJECT_ROOT / "results/m4_hourly_data_quality_summary.yaml"
AUDIT_CHECKS_PATH = PROJECT_ROOT / "results/m4_hourly_audit_checks.csv"

PROCESSED_PATH = OUTPUT_ROOT / "data/processed/m4_hourly_long.parquet"
MANIFEST_PATH = OUTPUT_ROOT / "results/m4_hourly_split_manifest.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_split_checks.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/m4_hourly_split_summary.yaml"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_four_stage_split.png"
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_split_report.json"
...
def main() -> None:
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "source_metadata_is_m4_hourly_with_48_hour_horizon",
        ...
        (
            "each_project_test_partition_contains_at_least_official_horizon",
            minimum_counts["test"] >= EXPECTED_HORIZON,
            f"minimum_test={minimum_counts['test']}; official_horizon={EXPECTED_HORIZON}",
        ),
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly split audit failed: {message}")
    ...
    summary = {
        ...
        "official_m4_forecast_horizon_hours": EXPECTED_HORIZON,
        "project_test_definition": "final 15% under frozen cross-dataset protocol",
        "official_horizon_note": (
            "The 48-hour archive horizon is recorded and is fully contained in every "
            "project test partition; it does not replace the preregistered 15% test split."
        ),
    ...
    axes[0, 0].set_title("Exact split counts for the two length groups")
    ...
    example_indices = [
        int(np.flatnonzero(lengths == length)[0])
    ...
    fig.suptitle(
        "M4 Hourly: leakage-safe 60/15/10/15 chronological split",
        fontsize=15,
    )
    ...
    print("官方预测范围：", EXPECTED_HORIZON, "小时（已完整包含在每条测试段中）")
