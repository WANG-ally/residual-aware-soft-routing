#!/usr/bin/env python3
"""Train M4 Hourly pretest routing baselines and ablations.
...
FEATURE_PATH = PROJECT_ROOT / "results/m4_hourly_router_features.parquet"
FEATURE_MANIFEST_PATH = (
    PROJECT_ROOT / "results/m4_hourly_router_feature_manifest.csv"
)
ROUTER_PARAMETER_PATH = (
    PROJECT_ROOT / "results/m4_hourly_selected_soft_router_params.yaml"
)
FOLD_MANIFEST_PATH = (
    PROJECT_ROOT / "results/m4_hourly_soft_router_fold_manifest.csv"
)
FOLD_SAMPLE_PATH = (
    PROJECT_ROOT / "results/m4_hourly_soft_router_sample_positions.parquet"
)
FINAL_SAMPLE_PATH = (
    PROJECT_ROOT / "results/m4_hourly_final_router_sample_positions.parquet"
)
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"
FREEZE_PATH = PROJECT_ROOT / "results/m4_hourly_pretest_freeze_manifest.json"

PROTOCOL_PATH = OUTPUT_ROOT / "results/m4_hourly_baseline_protocol_supplement.yaml"
DETAIL_PATH = OUTPUT_ROOT / "results/m4_hourly_baseline_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/m4_hourly_baseline_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/m4_hourly_selected_baseline_params.yaml"
CALIBRATION_PATH = (
    OUTPUT_ROOT / "results/m4_hourly_baseline_calibration_scores.parquet"
)
CHECKS_PATH = OUTPUT_ROOT / "results/m4_hourly_baseline_training_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/m4_hourly_baseline_validation.png"
REPORT_PATH = OUTPUT_ROOT / "logs/m4_hourly_baseline_training_report.json"
MODEL_DIR = OUTPUT_ROOT / "models"

DATASET_ID = "m4_hourly"
N_FOLDS = 3
MAX_TRAINING_SAMPLE_ROWS = 100_000
EXPECTED_ROUTER_ROWS = 49_713
EXPECTED_CALIBRATION_ROWS = 37_006
EXPECTED_FREEZE_ID = "affd9ef8154310f2863286ee0a07aeb58bc13b463178d0326f3079bbf8c5e03f"
...
def main() -> None:
    ...
    protected_outputs = [
        SELECTED_PATH,
        CALIBRATION_PATH,
        MODEL_DIR / "m4_hourly_hard_logistic_same_features.joblib",
        MODEL_DIR / "m4_hourly_hard_random_forest_same_features.joblib",
        MODEL_DIR / "m4_hourly_soft_targets_only.joblib",
        MODEL_DIR / "m4_hourly_residual_features_only.joblib",
        MODEL_DIR / "m4_hourly_class_weight_only.joblib",
    ]
    ...
    if (
        ...
        raise AssertionError("M4 Hourly baseline feature sets are invalid")
    ...
    protocol = {
        ...
        "fold_boundaries": "identical to m4_hourly_soft_router_fold_manifest.csv",
        "training_sample": {
            "maximum_rows_per_fold": MAX_TRAINING_SAMPLE_ROWS,
            "sample_count_rule": "min(all available prefix rows, 100000)",
            "positions": "identical to M4 Hourly full-router tuning",
            "final_positions": "identical to M4 Hourly final full-router fit",
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        partial_path = DETAIL_PATH.with_name(
            "m4_hourly_baseline_rolling_validation.partial.csv"
        )
    ...
    for method, (feature_names, model_kind) in model_definitions.items():
        ...
        model_path = MODEL_DIR / f"m4_hourly_{method}.joblib"
        bundle = {
            "dataset_id": DATASET_ID,
            "method": method,
            "model": model,
    ...
    formal_outputs = [
        PROJECT_ROOT / "results/m4_hourly_test_predictions.parquet",
        PROJECT_ROOT / "results/m4_hourly_test_aggregate_metrics.csv",
        PROJECT_ROOT / "logs/m4_hourly_formal_test_access_receipt.json",
    ]
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            ...
            len(model_hashes) == 5
            and all(
                (MODEL_DIR / f"m4_hourly_{method}.joblib").is_file()
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"M4 Hourly baseline training failed: {message}")
    ...
    figure.suptitle(
        "M4 Hourly pretest baseline and ablation selection", fontsize=14
    )
    ...
    print()
    print("M4 Hourly 预测试基线与消融模型训练全部通过")
    print("第一次冻结完整性检查：通过")
