#!/usr/bin/env python3
"""Causal context and past-residual features for Electricity Hourly routing."""
...
PREDICTION_PATH = PROJECT_ROOT / "results/electricity_hourly_pretest_predictions.parquet"
SCALER_PATH = PROJECT_ROOT / "results/electricity_hourly_scaler_parameters.csv"
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"
BASE_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_base_model_fit_checks.csv"
BASE_METADATA_PATH = PROJECT_ROOT / "results/electricity_hourly_base_model_fit_metadata.yaml"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/electricity_hourly_split_summary.yaml"

FEATURE_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_features.parquet"
MANIFEST_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_feature_manifest.csv"
COUNT_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_feature_counts.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_feature_checks.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_feature_summary.yaml"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_router_target_distribution.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_router_feature_report.json"
...
LONG_CONTEXT = 168
EXPECTED_SERIES = 321
EXPECTED_RAW_ROWS = 7_176_918
EXPECTED_PREDICTION_ROWS = 2_110_896
EXPECTED_ROUTER_ROWS_AFTER_WARMUP = 1_261_530
EXPECTED_CALIBRATION_ROWS = 844_230
EXPECTED_RETAINED_ROWS = 2_105_760
EXPECTED_RAW_ROWS_PER_SERIES = 22_358
EXPECTED_PREDICTION_ROWS_PER_SERIES = 6_576
EXPECTED_RETAINED_ROWS_PER_SERIES = 6_560
...
def main() -> None:
    ...
    if expected_raw_rows != EXPECTED_RAW_ROWS:
        raise AssertionError(f"Registered raw row count changed: {expected_raw_rows}")
    ...
    if expected_prediction_rows != EXPECTED_PREDICTION_ROWS:
        raise AssertionError(
            f"Registered prediction row count changed: {expected_prediction_rows}"
        )
    ...
    exact_raw_series_lengths = True
    exact_raw_split_counts = True
    for series_id_raw, group in raw.groupby(
        ...
        raw_indices_contiguous &= np.array_equal(
            indices, np.arange(len(group), dtype=np.int64)
        )
        values = group["value"].to_numpy(dtype=np.float64)
        exact_raw_series_lengths &= len(group) == EXPECTED_RAW_ROWS_PER_SERIES
        exact_raw_split_counts &= group["split"].astype(str).value_counts().to_dict() == {
            "base_train": 15_782,
            "router_train": 3_946,
            "calibration": 2_630,
        }
    ...
    if len(raw_series) != EXPECTED_SERIES:
        raise AssertionError(f"Expected {EXPECTED_SERIES} series, got {len(raw_series)}")
    ...
    del residual_feature_data, grouped
    gc.collect()
    ...
    retained_rows_per_series = (
        features.groupby("series_id", observed=True).size().to_numpy(dtype=np.int64)
    )
    if len(features) != EXPECTED_RETAINED_ROWS:
        raise AssertionError(
            f"Retained feature row mismatch: {len(features)} vs {EXPECTED_RETAINED_ROWS}"
        )
    ...
    descriptions = {
        ...
        "mean_24_scaled": "Mean of the 24 observations immediately before target",
        "trend_24_scaled": "Change from target-25 to target-1",
        "volatility_24_scaled": "Population standard deviation over previous 24 hours",
        "trend_168_scaled": "Change from target-169 to target-1",
        "volatility_168_scaled": "Population standard deviation over previous 168 hours",
        ...
        "hour_of_day_sin": "Sine encoding of known target hour",
        "hour_of_day_cos": "Cosine encoding of known target hour",
        "day_of_week_sin": "Sine encoding of known target weekday",
        "day_of_week_cos": "Cosine encoding of known target weekday",
    }
        ...
        manifest_records.append(
            {
                ...
                "maximum_observed_value_time_relative_to_target": (
                    -1 if not name.startswith(("hour_of_day", "day_of_week")) else np.nan
                ),
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        ("all_321_series_are_present", len(raw_series) == EXPECTED_SERIES, str(len(raw_series))),
        (
            "every_raw_series_has_registered_equal_split_lengths",
            exact_raw_series_lengths and exact_raw_split_counts,
            "base/router/calibration=15782/3946/2630",
        ),
        (
            "every_prediction_series_has_registered_row_count",
            bool(
                len(prediction_rows_per_series) == EXPECTED_SERIES
                and np.all(
                    prediction_rows_per_series == EXPECTED_PREDICTION_ROWS_PER_SERIES
                )
            ),
            f"expected_each={EXPECTED_PREDICTION_ROWS_PER_SERIES}",
        ),
        ...
        (
            "every_series_has_registered_retained_row_count",
            bool(
                len(retained_rows_per_series) == EXPECTED_SERIES
                and np.all(
                    retained_rows_per_series == EXPECTED_RETAINED_ROWS_PER_SERIES
                )
            ),
            f"expected_each={EXPECTED_RETAINED_ROWS_PER_SERIES}",
        ),
        (
            "feature_manifest_has_46_features",
            len(manifest) == 46
            and len(feature_names) == 46
            and len(set(feature_names)) == 46,
            f"manifest={len(manifest)}; features={len(feature_names)}",
        ),
        (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly router-feature audit failed: {message}")
    ...
    summary = {
        ...
        "black_box_point_win_rate": {
            split_name: float(win_rate.loc[split_name])
            for split_name in ROUTING_SPLITS
        },
    ...
    axes[0, 0].bar(
        win_rate.index, win_rate.values, color=["#4C78A8", "#F28E2B"]
    )
    ...
    fig.suptitle("Electricity Hourly causal routing features and targets", fontsize=15)
    ...
    print("Electricity Hourly 路由特征生成全部通过")
