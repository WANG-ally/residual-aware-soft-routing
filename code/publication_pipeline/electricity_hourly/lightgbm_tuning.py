#!/usr/bin/env python3
"""Base-only rolling tuning for Electricity Hourly LightGBM."""
...
RIDGE_FOLD_PATH = PROJECT_ROOT / "results/electricity_hourly_ridge_fold_manifest.csv"
RIDGE_FOLD_SCALER_PATH = PROJECT_ROOT / "results/electricity_hourly_ridge_fold_scalers.csv"
RIDGE_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_ridge_tuning_checks.csv"

DETAIL_PATH = OUTPUT_ROOT / "results/electricity_hourly_lightgbm_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/electricity_hourly_lightgbm_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/electricity_hourly_selected_lightgbm_params.yaml"
SAMPLING_MANIFEST_PATH = OUTPUT_ROOT / "results/electricity_hourly_lightgbm_sampling_manifest.csv"
SAMPLING_POSITIONS_PATH = OUTPUT_ROOT / "results/electricity_hourly_lightgbm_sample_positions.parquet"
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_lightgbm_tuning_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_lightgbm_tuning.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_lightgbm_tuning_report.json"
...
EXPECTED_VALIDATION_COUNT_PER_SERIES = 168
EXPECTED_VALIDATION_SAMPLES = EXPECTED_SERIES * EXPECTED_VALIDATION_COUNT_PER_SERIES
EXPECTED_SAMPLED_ROWS_PER_FOLD_WINDOW = 100_000
N_FOLDS = 3
SEASONAL_PERIOD = 24
MAX_TUNING_TRAIN_TARGETS = 100_000
SAMPLING_METHOD = "global midpoint systematic sampling over concatenated series targets"
...
def systematic_sample_by_series(
    ...
    counts = np.asarray(
        [available_counts[series_id] for series_id in series_order],
        dtype=np.int64,
    )
    if np.any(counts <= 0):
    ...
    if len(np.unique(global_positions)) != sample_size:
        raise AssertionError("Systematic sample contains duplicate global positions")
...
def main() -> None:
    ...
    feature_fraction_grid = [float(value) for value in black_box["feature_fraction"]]
    parameter_grid = list(
        product(
            num_leaves_grid,
            learning_rate_grid,
    ...
    if len(parameter_grid) != 16:
        raise AssertionError(
            f"Expected 16 tree configurations, got {len(parameter_grid)}"
        )
    ...
    if base_data.empty or observed_splits != {"base_train"}:
        raise AssertionError(
            f"Tuning input must contain only base_train: {observed_splits}"
        )
    if len(base_data) != EXPECTED_BASE_ROWS:
        raise AssertionError(
            f"Expected {EXPECTED_BASE_ROWS} base_train rows, got {len(base_data)}"
        )
    ...
    if len(series_values) != EXPECTED_SERIES:
        raise AssertionError(
            f"Expected {EXPECTED_SERIES} series, got {len(series_values)}"
        )
    observed_lengths = {len(values) for values in series_values.values()}
    if observed_lengths != {EXPECTED_BASE_LENGTH}:
        raise AssertionError(
            f"Unexpected base_train series lengths: {sorted(observed_lengths)}"
        )

    if sorted(fold_manifest["fold"].unique().tolist()) != [1, 2, 3]:
        raise AssertionError("Ridge fold manifest does not contain folds 1, 2, 3")
    if set(fold_manifest["dataset_id"].astype(str)) != {DATASET_ID}:
        raise AssertionError("Ridge fold manifest has an unexpected dataset_id")
    if set(fold_scalers["dataset_id"].astype(str)) != {DATASET_ID}:
        raise AssertionError("Ridge fold scalers have an unexpected dataset_id")
    expected_artifact_rows = EXPECTED_SERIES * N_FOLDS
    if len(fold_manifest) != expected_artifact_rows:
        raise AssertionError(
            f"Expected {expected_artifact_rows} fold rows, got {len(fold_manifest)}"
        )
    if len(fold_scalers) != expected_artifact_rows:
        raise AssertionError(
            f"Expected {expected_artifact_rows} scaler rows, got {len(fold_scalers)}"
        )
    if set(fold_manifest["series_id"].astype(str)) != set(series_order):
        raise AssertionError("Ridge fold manifest series do not match base_train")
    if set(fold_scalers["series_id"].astype(str)) != set(series_order):
        raise AssertionError("Ridge fold scaler series do not match base_train")
    ...
    observed_validation_counts = set(
        fold_manifest["validation_count"].astype(int).unique().tolist()
    )
    exact_fold_schedule_ok = all(
        len(observed_train_counts.get(fold, [])) == 1
        and int(observed_train_counts[fold][0]) == expected_count
        for fold, expected_count in EXPECTED_FOLD_TRAIN_COUNTS.items()
    ) and observed_validation_counts == {EXPECTED_VALIDATION_COUNT_PER_SERIES}
    if not exact_fold_schedule_ok:
        raise AssertionError(
            "Ridge fold schedule does not match the registered Electricity snapshot"
        )
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        for window in windows:
            ...
            expected_train_samples = min(
                total_available, MAX_TUNING_TRAIN_TARGETS
            )
            if expected_train_samples != EXPECTED_SAMPLED_ROWS_PER_FOLD_WINDOW:
                raise AssertionError(
                    "Every Electricity fold-window group should reach the 100000-row cap"
                )
            expected_samples_by_fold_window[(fold, window)] = expected_train_samples
            if len(global_positions) != expected_train_samples:
                raise AssertionError("Global systematic sample size mismatch")
            ...
            for series_id in series_order:
                ...
                train_x_parts.append(
                    window_view[targets - window, ::-1].astype(
                    ...
                    pd.DataFrame(
                        {
                            ...
                            "window": np.full(
                                sampled_count, window, dtype=np.int16
                            ),
                            "series_id": series_id,
            ...
            if (
                len(train_x) != expected_train_samples
                or sample_order_cursor != expected_train_samples
            ):
    ...
    expected_sample_position_rows = int(sum(expected_samples_by_fold_window.values()))
    sample_group_counts = (
        sample_positions.groupby(["fold", "window"], observed=True)
        .size()
        .to_dict()
    )
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "base_row_count_matches_registered_split",
            len(base_data) == EXPECTED_BASE_ROWS,
            f"actual={len(base_data)}; expected={EXPECTED_BASE_ROWS}",
        ),
        (
            "all_base_series_have_registered_equal_length",
            observed_lengths == {EXPECTED_BASE_LENGTH},
            str(sorted(observed_lengths)),
        ),
        (
        ...
        (
            ...
            shared_fold_artifacts_ok,
            str(shared_fold_artifacts_ok),
        ),
        (
            "ridge_fold_schedule_matches_registered_snapshot",
            exact_fold_schedule_ok,
            (
                f"train_counts={EXPECTED_FOLD_TRAIN_COUNTS}; "
                f"validation_count={EXPECTED_VALIDATION_COUNT_PER_SERIES}"
            ),
        ),
        (
        ...
        (
            ...
            len(sample_positions) == expected_sample_position_rows
        ...
        (
            ...
            f"passed={full_validation_used}; expected_per_fit={EXPECTED_VALIDATION_SAMPLES}",
        ),
        (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly LightGBM tuning failed: {message}")
    ...
    selected = {
        ...
        "fold_boundaries": "identical to electricity_hourly_ridge_fold_manifest.csv",
        "training_sample": {
    ...
    axes[0].barh(labels, top["mean_RMSSE"], color="#4C78A8")
    ...
    window_effect = (
        ...
        .agg(
            best_RMSSE=("mean_RMSSE", "min"),
            median_RMSSE=("mean_RMSSE", "median"),
        )
        .sort_values("window")
    )
    ...
    fig.suptitle("Electricity Hourly LightGBM tuning within base_train only", fontsize=15)
    ...
    print()
    print("Electricity Hourly LightGBM 滚动调参全部通过")
