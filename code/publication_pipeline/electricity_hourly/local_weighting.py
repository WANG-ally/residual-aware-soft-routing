#!/usr/bin/env python3
"""Local-weighting ablation and final Electricity Hourly soft-router fit.
...
FEATURE_MANIFEST_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_router_feature_manifest.csv"
)
FEATURE_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_router_feature_checks.csv"
SELECTED_ROUTER_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_selected_soft_router_params.yaml"
)
ROUTER_DETAIL_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_soft_router_rolling_validation.csv"
)
ROUTER_FOLD_PATH = PROJECT_ROOT / "results/electricity_hourly_soft_router_fold_manifest.csv"
ROUTER_SAMPLE_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_soft_router_sample_positions.parquet"
)
ROUTER_CHECKS_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_soft_router_tuning_checks.csv"
)
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"

DETAIL_PATH = OUTPUT_ROOT / "results/electricity_hourly_local_weighting_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/electricity_hourly_local_weighting_tuning_summary.csv"
SELECTED_LOCAL_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_selected_local_weighting_params.yaml"
)
FINAL_SAMPLE_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_final_router_sample_positions.parquet"
)
NEIGHBOR_MANIFEST_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_local_weighting_neighbor_manifest.csv"
)
COEFFICIENT_PATH = OUTPUT_ROOT / "results/electricity_hourly_router_coefficients.csv"
CALIBRATION_SCORE_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_calibration_router_scores.parquet"
)
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_local_weighting_checks.csv"
MODEL_PATH = OUTPUT_ROOT / "models/electricity_hourly_soft_router.joblib"
FIGURE_PATH = (
    OUTPUT_ROOT / "figures/electricity_hourly_router_coefficients_and_scores.png"
)
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_local_weighting_report.json"
...
EXPECTED_SERIES = 321
EXPECTED_SELECTED_FEATURES = 16
EXPECTED_ROUTER_ROWS = 1_261_530
EXPECTED_ROUTER_ROWS_PER_SERIES = 3_930
EXPECTED_CALIBRATION_ROWS = 844_230
EXPECTED_CALIBRATION_ROWS_PER_SERIES = 2_630
...
PRETEST_ALPHA_EXTENSION = [4.0, 8.0, 16.0, 32.0]
EXPECTED_LOCAL_COMBINATIONS = 24
EXPECTED_ROLLING_FITS = 72
...
def main() -> None:
    ...
    if not upstream_checks_passed:
        raise AssertionError("An upstream Electricity Hourly audit did not pass")
    ...
    registered_alpha_grid = [
        float(value) for value in config["local_weighting"]["alpha_grid"]
    ]
    alpha_grid = [*registered_alpha_grid, *PRETEST_ALPHA_EXTENSION]
    maximum_multiplier = float(
        config["local_weighting"]["maximum_multiplier"]
    )
    if k_grid != [5, 11, 21] or registered_alpha_grid != REGISTERED_ALPHA_GRID:
        raise AssertionError(
            "Unexpected registered local-weighting grid: "
            f"k={k_grid}, alpha={registered_alpha_grid}"
        )
    ...
    if not (
        ...
        raise AssertionError("Frozen Electricity soft-router parameters changed")
    ...
    if not (
        ...
        raise AssertionError("Electricity router series lengths changed")
    ...
    if len(series_order) != EXPECTED_SERIES:
        raise AssertionError(
            f"Expected {EXPECTED_SERIES} series, found {len(series_order)}"
        )
    ...
    ) and set(fold_manifest["validation_count"].astype(int).unique().tolist()) == {168}
    if not exact_fold_schedule_ok:
        raise AssertionError("Saved Electricity router fold schedule changed")
    saved_sample_counts = (
        saved_sample_positions.groupby("fold", observed=True).size().to_dict()
    )
    if not (
        len(saved_sample_counts) == N_FOLDS
        and set(saved_sample_counts.values()) == {EXPECTED_SAMPLE_ROWS}
    ):
        raise AssertionError("Saved Electricity router samples changed")
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        if len(sampled_train_indices) != expected_sample_rows:
            raise AssertionError(
                f"Fold {fold} sample rows={len(sampled_train_indices)}; "
                f"expected={expected_sample_rows}"
            )
        if expected_sample_rows != EXPECTED_SAMPLE_ROWS:
            raise AssertionError("Every Electricity fold must use 100000 rows")
        if len(validation_indices) != EXPECTED_VALIDATION_ROWS_PER_FOLD:
            raise AssertionError("Electricity validation row count changed")
    ...
    selected_alpha_below_extended_upper_boundary = bool(
        best_alpha < max(alpha_grid)
    )
    if not selected_alpha_below_extended_upper_boundary:
        raise AssertionError(
            "Extended local-weighting alpha grid still selected its upper boundary"
        )
    ...
    # standardization and a saved deterministic sample capped at 100,000 rows.
    ...
    artifact_probe_raw = router.iloc[final_sample_indices[:8]][
        feature_names
    ...
    calibration_rows_per_series = (
        calibration.groupby("series_id", observed=True).size().to_numpy(dtype=np.int64)
    )
    if not (
        len(calibration) == EXPECTED_CALIBRATION_ROWS
        and len(calibration_rows_per_series) == EXPECTED_SERIES
        and np.all(
            calibration_rows_per_series == EXPECTED_CALIBRATION_ROWS_PER_SERIES
        )
    ):
        raise AssertionError("Electricity calibration row counts changed")
    ...
    if expected_fits != EXPECTED_ROLLING_FITS:
        raise AssertionError(f"Expected rolling-fit count changed: {expected_fits}")
    if expected_combinations != EXPECTED_LOCAL_COMBINATIONS:
        raise AssertionError(
            f"Expected local-weighting combination count changed: {expected_combinations}"
        )
    finite_columns = [
    ...
    checks_items: list[tuple[str, bool, str]] = [
        ...
        (
            "every_router_series_has_registered_row_count",
            bool(
                len(router_rows_per_series) == EXPECTED_SERIES
                and np.all(
        ...
        (
            "selected_router_has_registered_16_features",
            len(feature_names) == EXPECTED_SELECTED_FEATURES,
        ...
        (
            "every_fold_reuses_exactly_100000_saved_rows",
            len(saved_sample_counts) == N_FOLDS
            and set(saved_sample_counts.values()) == {EXPECTED_SAMPLE_ROWS},
            str(saved_sample_counts),
        ),
        (
        ...
        (
            "alpha_grid_extension_is_router_only_and_fixed_before_official_run",
            registered_alpha_grid == REGISTERED_ALPHA_GRID
            and PRETEST_ALPHA_EXTENSION == [4.0, 8.0, 16.0, 32.0],
            (
                f"registered={registered_alpha_grid}; "
                f"extension={PRETEST_ALPHA_EXTENSION}"
            ),
        ),
        (
            "selected_alpha_is_below_extended_upper_boundary",
            selected_alpha_below_extended_upper_boundary,
            f"selected={best_alpha}; extended_max={max(alpha_grid)}",
        ),
        (
        ...
        (
            "every_validation_fit_uses_complete_53928_rows",
            set(details["validation_rows"].astype(int).unique().tolist())
            == {EXPECTED_VALIDATION_ROWS_PER_FOLD},
            str(sorted(details["validation_rows"].astype(int).unique().tolist())),
        ),
        (
        ...
        (
            "final_sample_count_is_minimum_of_available_and_cap",
            len(final_sample_positions)
            == min(len(router), MAX_TRAINING_SAMPLE_ROWS),
            (
                f"actual={len(final_sample_positions)}; "
                f"expected={min(len(router), MAX_TRAINING_SAMPLE_ROWS)}"
            ),
        ),
        (
        ...
        (
            "saved_final_router_bundle_reloads_identically",
            model_artifact_reload_ok,
            str(model_artifact_reload_ok),
        ),
        (
            "calibration_has_expected_rows",
            len(calibration_scores) == EXPECTED_CALIBRATION_ROWS,
            f"rows={len(calibration_scores)}",
        ),
        (
            "every_calibration_series_has_registered_row_count",
            bool(
                len(calibration_rows_per_series) == EXPECTED_SERIES
                and np.all(
                    calibration_rows_per_series
                    == EXPECTED_CALIBRATION_ROWS_PER_SERIES
                )
            ),
            f"expected_each={EXPECTED_CALIBRATION_ROWS_PER_SERIES}",
        ),
        (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly local weighting failed: {message}")
    ...
    selected_local = {
        ...
        "parameter_grid": {
            "k": k_grid,
            "registered_alpha": registered_alpha_grid,
            "pretest_boundary_extension": PRETEST_ALPHA_EXTENSION,
            "evaluated_alpha": alpha_grid,
            "maximum_multiplier": maximum_multiplier,
        },
        "grid_extension_amendment": {
            "status": "documented_before_registered_local_weighting_run",
            "trigger": (
                "An isolated router_train-only rolling comparison selected the "
                "registered upper boundary alpha=2 while constrained loss was "
                "still decreasing."
            ),
            "rule": (
                "Append four doubling values and accept the extension only if "
                "the selected alpha is below the extended upper boundary."
            ),
    ...
    axes[0].set_title("Final Electricity Hourly soft-router coefficients")
    ...
    figure.suptitle(
        "Electricity Hourly final soft router: coefficients and calibration scores",
        fontsize=14,
    )
    ...
    print()
    print("Electricity Hourly 局部加权与最终路由器训练全部通过")
