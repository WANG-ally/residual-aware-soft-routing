#!/usr/bin/env python3
"""Freeze or execute the one-time Electricity Hourly formal evaluator.
...
EXPECTED_FINAL_FREEZE_ID = (
    "3664f0711faa5625278e05ac2d45a03c4d7561667f4b2579830c624954386f29"
)
EXPECTED_FINAL_FREEZE_FILES = 85
EXPECTED_SERIES = 321
EXPECTED_CALIBRATION_ROWS = 844_230
EXPECTED_CALIBRATION_STEPS = 2_630
EXPECTED_CALIBRATION_ROWS_PER_SERIES = 2_630
EXPECTED_TEST_ROWS = 1_266_666
EXPECTED_TEST_STEPS = 3_946
EXPECTED_TEST_ROWS_PER_SERIES = 3_946
EXPECTED_PRETEST_ROWS = 2_110_896
EXPECTED_PRETEST_ROWS_PER_SERIES = 6_576
SEASONAL_PERIOD = 24
LONG_CONTEXT = 168
ROUTER_RESIDUAL_LAG = 1
ROUTER_FEATURE_COUNT = 16
MAX_RESIDUAL_LAG = 16
ROLLING_COVERAGE_HOURS = 48

FINAL_LOCK_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_final_pretest_lock_manifest.json"
)
AUTHORIZATION_PLAN_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_final_test_authorization_plan.yaml"
)
DATA_PATH = PROJECT_ROOT / "data/processed/electricity_hourly_long.parquet"
SPLIT_MANIFEST_PATH = PROJECT_ROOT / "results/electricity_hourly_split_manifest.csv"
SCALER_PATH = PROJECT_ROOT / "results/electricity_hourly_scaler_parameters.csv"
PRETEST_PATH = PROJECT_ROOT / "results/electricity_hourly_pretest_predictions.parquet"
ROUTER_FEATURE_PATH = PROJECT_ROOT / "results/electricity_hourly_router_features.parquet"
CALIBRATION_ROUTER_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_calibration_router_scores.parquet"
)
CALIBRATION_CONTROLLER_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_calibration_controller_decisions.parquet"
)
BASELINE_CALIBRATION_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_baseline_calibration_scores.parquet"
)
RIDGE_PARAM_PATH = PROJECT_ROOT / "results/electricity_hourly_selected_ridge_params.yaml"
LGBM_PARAM_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_selected_lightgbm_params.yaml"
)
CONTROLLER_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_selected_coverage_controller.yaml"
)
BASELINE_PARAM_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_selected_baseline_params.yaml"
)
RIDGE_MODEL_PATH = PROJECT_ROOT / "models/electricity_hourly_ridge.joblib"
LGBM_MODEL_PATH = PROJECT_ROOT / "models/electricity_hourly_lightgbm.joblib"
FULL_ROUTER_MODEL_PATH = PROJECT_ROOT / "models/electricity_hourly_soft_router.joblib"

AUTHORIZATION_PATH = OUTPUT_ROOT / "logs/electricity_hourly_evaluator_authorization.json"
RECEIPT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_formal_test_access_receipt.json"
PREDICTION_PATH = OUTPUT_ROOT / "results/electricity_hourly_test_predictions.parquet"
PER_SERIES_PATH = OUTPUT_ROOT / "results/electricity_hourly_test_per_series_metrics.csv"
AGGREGATE_PATH = OUTPUT_ROOT / "results/electricity_hourly_test_aggregate_metrics.csv"
COVERAGE_TRACE_PATH = OUTPUT_ROOT / "results/electricity_hourly_test_coverage_trace.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_test_method_comparison.png"
...
def verify_final_lock() -> dict:
    ...
    if (
        ...
        or int(manifest.get("file_count", -1))
        != EXPECTED_FINAL_FREEZE_FILES
...
def load_artifacts() -> dict:
    ...
    if (
        ...
        or int(full_router["residual_lag"]) != ROUTER_RESIDUAL_LAG
        or len(full_router["feature_names"]) != ROUTER_FEATURE_COUNT
        ...
        is not True
        or int(controller.get("rolling_diagnostic_window_hours", -1)) != 48
    ):
        raise ValueError("Frozen Electricity Hourly parameters differ from the test plan")

    scalers = pd.read_csv(SCALER_PATH)
    split_manifest = pd.read_csv(SPLIT_MANIFEST_PATH)
    pretest = pd.read_parquet(PRETEST_PATH)
    pretest_lengths = pretest.groupby(
        "series_id", sort=False, observed=True
    ).size()
    if (
        scalers["source_split"].ne("base_train").any()
        or len(scalers) != EXPECTED_SERIES
        or scalers["series_id"].astype(str).nunique() != EXPECTED_SERIES
        or len(split_manifest) != EXPECTED_SERIES
        or split_manifest["series_id"].astype(str).nunique() != EXPECTED_SERIES
        or not split_manifest["total_length"].eq(26_304).all()
        or not split_manifest["base_train_count"].eq(15_782).all()
        or not split_manifest["router_train_count"].eq(3_946).all()
        or not split_manifest["calibration_count"].eq(2_630).all()
        or not split_manifest["test_count"].eq(3_946).all()
        or set(pretest["split"].astype(str)) != {"router_train", "calibration"}
        or len(pretest) != EXPECTED_PRETEST_ROWS
        or len(pretest_lengths) != EXPECTED_SERIES
        or int(pretest_lengths.min()) != EXPECTED_PRETEST_ROWS_PER_SERIES
        or int(pretest_lengths.max()) != EXPECTED_PRETEST_ROWS_PER_SERIES
    ):
...
def build_segment(
    ...
    """Build causal predictions/features for an equal-length segment.
    ...
    if len(frames) != EXPECTED_SERIES:
        raise ValueError(
            f"Expected {EXPECTED_SERIES} series, observed {len(frames)}"
        )
    ...
    for series_id, frame in frames.items():
        ...
        expected_targets = (
            ...
            if segment_name == "calibration"
            else EXPECTED_TEST_ROWS_PER_SERIES
        )
        if len(targets) != expected_targets:
            raise ValueError(
                f"{series_id} {segment_name} rows={len(targets)}; "
    ...
    if (
        len(feature_names) != ROUTER_FEATURE_COUNT
...
def dry_run_calibration(artifacts: dict) -> dict:
    ...
    calibration_lengths = computed.groupby(
        "series_id", sort=False, observed=True
    ).size()
    calibration_batch_sizes = computed.groupby(
        "relative_segment_step", sort=True, observed=True
    ).size()
    if (
        len(computed) != EXPECTED_CALIBRATION_ROWS
        or computed["series_id"].nunique() != EXPECTED_SERIES
        or int(calibration_lengths.min())
        != EXPECTED_CALIBRATION_ROWS_PER_SERIES
        or int(calibration_lengths.max())
        != EXPECTED_CALIBRATION_ROWS_PER_SERIES
        or computed["relative_segment_step"].nunique()
        != EXPECTED_CALIBRATION_STEPS
        or int(calibration_batch_sizes.min()) != EXPECTED_SERIES
        or int(calibration_batch_sizes.max()) != EXPECTED_SERIES
    ...
    return {
        ...
        "minimum_active_series": int(
            calibration_batch_sizes.min()
        ),
        "maximum_active_series": int(
            calibration_batch_sizes.max()
        ),
        "tolerance": tolerance,
...
def compute_metrics(
    ...
    coverage_trace["rolling_48h_simple_count"] = coverage_trace.groupby(
    ...
    coverage_trace["rolling_48h_rows"] = coverage_trace.groupby(
        "method", sort=False, observed=True
    ...
    coverage_trace["rolling_48h_simple_coverage"] = (
        coverage_trace["rolling_48h_simple_count"]
        / coverage_trace["rolling_48h_rows"]
    )
    ...
    for method in METHODS:
        ...
        rolling = method_trace["rolling_48h_simple_coverage"].dropna()
        worst_rolling = (
        ...
        aggregate_records.append(
            {
                ...
                "worst_48h_weighted_coverage_violation": worst_rolling,
...
def save_figure(
    ...
    axes[0].set_title("Electricity Hourly formal-test accuracy")
    ...
    for method, color in (
        ...
        axes[2].plot(
            ...
            part["rolling_48h_simple_coverage"],
            label=method,
            color=color,
        )
    ...
    axes[2].set_ylabel("Weighted rolling 48-hour Ridge coverage")
...
def freeze_evaluator() -> None:
    ...
    print()
    print("Electricity Hourly 正式测试执行器冻结与校准干运行全部通过")
...
def execute_formal_test() -> None:
    ...
    try:
        ...
        test_lengths = test_frame.groupby(
            "series_id", sort=False, observed=True
        ).size()
        test_batch_sizes = test_frame.groupby(
            "relative_segment_step", sort=True, observed=True
        ).size()
        if (
            len(test_frame) != EXPECTED_TEST_ROWS
            or test_frame["series_id"].nunique() != EXPECTED_SERIES
            or int(test_lengths.min()) != EXPECTED_TEST_ROWS_PER_SERIES
            or int(test_lengths.max()) != EXPECTED_TEST_ROWS_PER_SERIES
            or test_frame["relative_segment_step"].nunique()
            != EXPECTED_TEST_STEPS
            or int(test_batch_sizes.min()) != EXPECTED_SERIES
            or int(test_batch_sizes.max()) != EXPECTED_SERIES
    ...
    print()
    print("Electricity Hourly 唯一一次正式测试全部完成")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Freeze or execute the one-time Electricity Hourly formal evaluator"
    )
