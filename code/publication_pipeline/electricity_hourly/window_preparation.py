#!/usr/bin/env python3
"""Base-only scaling and causal window audit for Electricity Hourly.
...
EXPECTED_SERIES = 321
EXPECTED_TOTAL_ROWS = 8_443_584
EXPECTED_BASE_COUNT = 15_782
EXPECTED_WINDOW_COUNTS = {
    24: {
        "base_train": 5_058_318,
        "router_train": 1_266_666,
        "calibration": 844_230,
        "test": 1_266_666,
        "total": 8_435_880,
    },
    48: {
        "base_train": 5_050_614,
        "router_train": 1_266_666,
        "calibration": 844_230,
        "test": 1_266_666,
        "total": 8_428_176,
...
SPLIT_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_split_checks.csv"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/electricity_hourly_split_summary.yaml"

SCALER_PATH = OUTPUT_ROOT / "results/electricity_hourly_scaler_parameters.csv"
COUNT_PATH = OUTPUT_ROOT / "results/electricity_hourly_window_candidate_counts.csv"
SERIES_COUNT_PATH = OUTPUT_ROOT / "results/electricity_hourly_window_counts_by_series.csv"
PREVIEW_PATH = OUTPUT_ROOT / "results/electricity_hourly_window_preview.csv"
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_window_preparation_checks.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/electricity_hourly_window_preparation_summary.yaml"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_window_example.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_window_preparation_report.json"
...
def main() -> None:
    ...
    t183_scaler = scalers.set_index("series_id").loc["T183"]
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            ...
            and len(data) == int(split_summary["total_observations"]),
        ...
        (
            ...
            and len(scalers) == len(manifest)
            and scalers["series_id"].is_unique,
        ...
        (
            "every_scaler_uses_exactly_the_base_train_rows",
            scalers["source_start"].eq(0).all()
            and scalers["source_count"].eq(EXPECTED_BASE_COUNT).all()
            and scalers["source_end"].eq(EXPECTED_BASE_COUNT - 1).all(),
            f"expected_source_count={EXPECTED_BASE_COUNT}",
        ),
        (
        ...
        (
            "base_train_has_no_zero_iqr_fallback",
            int(scalers["zero_iqr_fallback"].sum()) == 0,
            f"fallback_count={int(scalers['zero_iqr_fallback'].sum())}",
        ),
        (
            "t183_full_series_audit_flag_does_not_leak_into_base_only_scaler",
            float(t183_scaler["iqr"]) > 0.0
            and not bool(t183_scaler["zero_iqr_fallback"]),
            (
                f"T183_base_iqr={float(t183_scaler['iqr'])}; "
                f"fallback={bool(t183_scaler['zero_iqr_fallback'])}"
            ),
        ),
        ...
        (
            "aggregate_window_counts_match_registered_snapshot",
            observed_window_counts == EXPECTED_WINDOW_COUNTS,
            f"observed={observed_window_counts}",
        ),
        (
            "preview_histories_end_before_targets",
            preview_series == "T1"
            and bool(
                (preview["history_end_index"] < preview["target_time_index"]).all()
            ),
            f"preview_series={preview_series}; preview_rows={len(preview)}",
        ),
        (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly window preparation failed: {message}")
    ...
    summary = {
        ...
        "scaler": {
            ...
            "t183_base_train_iqr": float(t183_scaler["iqr"]),
            "t183_note": (
                "The full-series audit IQR was zero, but the permitted base_train "
                "IQR is positive; no future-derived fallback decision is used."
            ),
        },
    ...
    axes[0, 0].set_title("Base-train scaler parameters across 321 series")
    ...
    fig.suptitle(
        "Electricity Hourly: base-only scaling and causal window preparation",
        fontsize=15,
    )
    ...
    print("Electricity Hourly 滑动窗口准备全部通过")
    ...
    print("T183 的 base_train IQR：", f"{float(t183_scaler['iqr']):.6f}")
