#!/usr/bin/env python3
"""Fit frozen Electricity Hourly base models and predict pre-test splits."""
...
WINDOW_COUNT_PATH = PROJECT_ROOT / "results/electricity_hourly_window_candidate_counts.csv"
SPLIT_SUMMARY_PATH = PROJECT_ROOT / "results/electricity_hourly_split_summary.yaml"
RIDGE_PARAMS_PATH = PROJECT_ROOT / "results/electricity_hourly_selected_ridge_params.yaml"
LGBM_PARAMS_PATH = PROJECT_ROOT / "results/electricity_hourly_selected_lightgbm_params.yaml"
RIDGE_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_ridge_tuning_checks.csv"
LGBM_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_lightgbm_tuning_checks.csv"

RIDGE_MODEL_PATH = OUTPUT_ROOT / "models/electricity_hourly_ridge.joblib"
LGBM_MODEL_PATH = OUTPUT_ROOT / "models/electricity_hourly_lightgbm.joblib"
PREDICTION_PATH = OUTPUT_ROOT / "results/electricity_hourly_pretest_predictions.parquet"
PER_SERIES_PATH = OUTPUT_ROOT / "results/electricity_hourly_pretest_per_series_metrics.csv"
AGGREGATE_PATH = OUTPUT_ROOT / "results/electricity_hourly_pretest_aggregate_metrics.csv"
METADATA_PATH = OUTPUT_ROOT / "results/electricity_hourly_base_model_fit_metadata.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_base_model_fit_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_pretest_predictions_T1.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_base_model_fit_report.json"
...
SEASONAL_PERIOD = 24
EXPECTED_SERIES = 321
EXPECTED_BASE_COUNT_PER_SERIES = 15_782
EXPECTED_ALLOWED_COUNT_PER_SERIES = 22_358
EXPECTED_PREDICTION_COUNT_PER_SERIES = 6_576
EXPECTED_ALLOWED_ROWS = 7_176_918
EXPECTED_PREDICTION_ROWS = 2_110_896
EXPECTED_LGBM_TRAIN_SAMPLES = 5_012_094
...
class CausalLagSequence(lgb.Sequence):
    """Expose all causal lag windows to LightGBM without a dense 3.14-GiB array."""
    ...
    def _row(self, index: int) -> np.ndarray:
        ...
        # LightGBM 4.6 requires random-access rows used for bin sampling to be
        # float64; range batches below remain float32 to limit memory.
...
def main() -> None:
    ...
    if lgbm_params.get("planned_final_training_scope") != "all eligible base_train targets":
        raise AssertionError("LightGBM final-training scope is not the frozen full target set")
    ...
    if expected_allowed_rows != EXPECTED_ALLOWED_ROWS:
        raise AssertionError(
            f"Registered allowed-row count changed: {expected_allowed_rows}"
        )
    if len(data) != expected_allowed_rows:
        raise AssertionError(
            f"Allowed row mismatch: data={len(data)}, expected={expected_allowed_rows}"
        )
    observed_allowed_rows = int(len(data))
    ...
    split_order_ok = True
    exact_series_lengths_ok = True
    exact_split_counts_ok = True
    split_codes = {name: number for number, name in enumerate(ALLOWED_SPLITS)}
    for group in series_frames.values():
        ...
        exact_series_lengths_ok &= len(group) == EXPECTED_ALLOWED_COUNT_PER_SERIES
        counts = group["split"].astype(str).value_counts().to_dict()
        exact_split_counts_ok &= counts == {
            "base_train": EXPECTED_BASE_COUNT_PER_SERIES,
            "router_train": 3_946,
            "calibration": 2_630,
        }
    ...
    if ridge_window != 168 or lgbm_window != 168:
        raise AssertionError(
            f"Expected selected 168-hour windows, got Ridge={ridge_window}, "
            f"LightGBM={lgbm_window}"
        )
    expected_ridge_samples = int(
        window_counts.loc[
            window_counts["window"] == ridge_window, "base_train"
        ].iloc[0]
    )
    expected_lgbm_samples = int(
        window_counts.loc[
            window_counts["window"] == lgbm_window, "base_train"
        ].iloc[0]
    )
    if expected_lgbm_samples != EXPECTED_LGBM_TRAIN_SAMPLES:
        raise AssertionError(
            f"Registered LightGBM target count changed: {expected_lgbm_samples}"
        )
    ...
    cursor = len(lgbm_sequence)
    if cursor != expected_lgbm_samples:
        raise AssertionError(
            f"LightGBM full training rows mismatch: {cursor} vs {expected_lgbm_samples}"
        )
    if len(lgbm_y) != expected_lgbm_samples:
        raise AssertionError("LightGBM label count does not match causal windows")
    ...
    for global_index in sequence_probe_indices:
        ...
        sequence_probe_ok &= np.array_equal(
            lgbm_sequence[global_index], expected_row
        )
        sequence_probe_ok &= bool(
            lgbm_y[global_index]
            == np.float32(
                scaled_base_series[series_index][lgbm_window + local_index]
            )
        )
    ...
    sequence_probe_ok &= np.array_equal(
        cross_boundary,
        np.stack(
            [lgbm_sequence[index] for index in range(cross_boundary_start, cross_boundary_start + 4)]
        ).astype(np.float32),
    )
    if not sequence_probe_ok:
        raise AssertionError("Out-of-core causal-window Sequence audit failed")
    print(
        f"[完成] 磁盘外式分批接口已构造；批大小={SEQUENCE_BATCH_SIZE}，"
        "未创建3.14 GiB密集矩阵",
        flush=True,
    )
    ...
    if not model_artifact_reload_ok:
        raise AssertionError("Saved LightGBM sklearn-compatible artifact failed reload audit")
    lgbm = reloaded_lgbm
    ...
    for series_id, group in series_frames.items():
        ...
        targets = group.loc[
            prediction_mask, "time_index"
        ].to_numpy(dtype=np.int64)
        causal_windows_ok &= bool(
            targets.min() >= max(ridge_window, lgbm_window)
            and np.all(targets - ridge_window >= 0)
            and np.all(targets - lgbm_window >= 0)
        )
        median, scale = scaler_lookup[series_id]
        scaled = (values - median) / scale
    ...
    prediction["ridge_residual"] = (
        prediction["y_true"] - prediction["ridge_prediction"]
    )
    ...
    prediction["black_box_better"] = (
        prediction["lightgbm_squared_error"]
    ...
    if expected_prediction_rows != EXPECTED_PREDICTION_ROWS:
        raise AssertionError(
            f"Registered pre-test prediction count changed: {expected_prediction_rows}"
        )
    ...
    per_series_prediction_counts = (
        prediction.groupby("series_id", observed=True).size().to_numpy(dtype=np.int64)
    )
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "input_row_count_matches_registered_splits",
            len(series_frames) == EXPECTED_SERIES
            and observed_allowed_rows == expected_allowed_rows,
            f"series={len(series_frames)}; rows={observed_allowed_rows}",
        ),
        (
            "every_series_has_registered_equal_split_lengths",
            exact_series_lengths_ok and exact_split_counts_ok,
            (
                f"allowed_per_series={EXPECTED_ALLOWED_COUNT_PER_SERIES}; "
                "base/router/calibration=15782/3946/2630"
            ),
        ),
        ...
        (
            ...
            cursor == expected_lgbm_samples,
            f"actual={cursor}; expected={expected_lgbm_samples}",
        ),
        (
        ...
        (
            "prediction_row_counts_match_manifest",
        ...
        (
            "every_series_has_registered_pretest_prediction_count",
            bool(
                len(per_series_prediction_counts) == EXPECTED_SERIES
                and np.all(
                    per_series_prediction_counts
                    == EXPECTED_PREDICTION_COUNT_PER_SERIES
                )
            ),
            (
                f"series={len(per_series_prediction_counts)}; "
                f"expected_each={EXPECTED_PREDICTION_COUNT_PER_SERIES}"
            ),
        ),
        (
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly base-model audit failed: {message}")
    ...
    example = prediction.loc[prediction["series_id"].astype(str) == "T1"]
    fig, axes = plt.subplots(2, 1, figsize=(13, 8), sharex=False)
    for ax, split_name in zip(axes, PREDICTION_SPLITS):
        part = example.loc[example["split"] == split_name]
        ax.plot(
            part["time_index"], part["y_true"], color="#222222", linewidth=1.5,
            label="actual",
        )
        ax.plot(
            part["time_index"], part["ridge_prediction"], color="#4C78A8",
            linewidth=1.1, label="ridge",
        )
        ax.plot(
            part["time_index"], part["lightgbm_prediction"], color="#F28E2B",
            linewidth=1.1, label="lightgbm",
        )
        ax.set_title(f"Electricity Hourly T1: {split_name}")
        ax.set_ylabel("Value")
        ax.grid(alpha=0.2)
    ...
    print()
    print("Electricity Hourly 基础模型训练和预测全部通过")
