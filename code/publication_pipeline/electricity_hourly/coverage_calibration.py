#!/usr/bin/env python3
"""Calibrate the Electricity Hourly adaptive coverage controller.

Only the saved calibration routing scores are read. No model is loaded or
refitted, and the test split is never accessed. Electricity Hourly series are
equal length; online batches are still aligned by hours elapsed since each
series entered its calibration segment so the test executor uses one rule."""
...
SCORE_PATH = PROJECT_ROOT / "results/electricity_hourly_calibration_router_scores.parquet"
LOCAL_PARAMETER_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_selected_local_weighting_params.yaml"
)
LOCAL_CHECKS_PATH = PROJECT_ROOT / "results/electricity_hourly_local_weighting_checks.csv"

TUNING_PATH = OUTPUT_ROOT / "results/electricity_hourly_coverage_controller_tuning.csv"
TRACE_PATH = OUTPUT_ROOT / "results/electricity_hourly_calibration_controller_trace.csv"
DECISION_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_calibration_controller_decisions.parquet"
)
THRESHOLD_PATH = OUTPUT_ROOT / "results/electricity_hourly_calibration_thresholds.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/electricity_hourly_selected_coverage_controller.yaml"
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_coverage_calibration_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_coverage_calibration.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_coverage_calibration_report.json"

DATASET_ID = "electricity_hourly"
EXPECTED_ROWS = 844_230
EXPECTED_SERIES = 321
EXPECTED_RELATIVE_STEPS = 2_630
EXPECTED_ROWS_PER_SERIES = 2_630
EXPECTED_BATCH_ROWS = 321
ROLLING_WINDOW_HOURS = 48
ALIGNMENT_RULE = "hours elapsed since each series entered calibration"
...
def equal_series_mean(
    values: np.ndarray, series_ids: np.ndarray
) -> float:
    frame = pd.DataFrame({"series_id": series_ids, "value": values})
    return float(
        frame.groupby("series_id", sort=False, observed=True)["value"]
        .mean()
        .mean()
    )
...
def simulate_controller(
    ...
    """Replay the controller without using observed targets in its update."""
    ...
    for relative_step, batch in calibration.groupby(
        ...
        batch_probability = batch["black_box_probability"].to_numpy(
            dtype=np.float64
        )
        threshold_before_decision = float(
            np.clip(base_threshold + bias, 0.0, 1.0)
        )
        ...
        selected_prediction = np.where(
            use_simple,
            batch["ridge_prediction"].to_numpy(dtype=np.float64),
            batch["lightgbm_prediction"].to_numpy(dtype=np.float64),
        )
        scaled_squared_error = (
            (
                batch["y_true"].to_numpy(dtype=np.float64)
                - selected_prediction
            )
            / batch["seasonal_naive_mae_scale"].to_numpy(dtype=np.float64)
        ) ** 2
        ...
        # The update uses routing decisions only.  y_true and prediction error
        # are not referenced in this expression.
        next_bias = float(
            np.clip(
    ...
    rolling_simple_count = trace["simple_count"].rolling(
        ROLLING_WINDOW_HOURS, min_periods=ROLLING_WINDOW_HOURS
    ).sum()
    rolling_rows = trace["batch_rows"].rolling(
        ROLLING_WINDOW_HOURS, min_periods=ROLLING_WINDOW_HOURS
    ).sum()
    trace["rolling_48h_simple_coverage"] = rolling_simple_count / rolling_rows
    valid_rolling = trace["rolling_48h_simple_coverage"].dropna()
    ...
    per_series = pd.DataFrame(
        {
            "series_id": all_series,
            "scaled_loss": all_loss,
            "simple": all_simple,
        }
    ...
    summary: dict[str, object] = {
        ...
        "absolute_coverage_violation": float(
            abs(overall_coverage - target_coverage)
        ),
        "equal_series_simple_coverage": float(
            per_series["simple_coverage"].mean()
        ),
        "mean_per_series_absolute_coverage_violation": float(
            np.mean(
                np.abs(
                    per_series["simple_coverage"].to_numpy()
                    - target_coverage
                )
            )
        ),
        ...
        "worst_48h_weighted_coverage_violation": worst_rolling_violation,
        "mean_batch_absolute_coverage_violation": float(
            np.mean(
                np.abs(
                    trace["simple_coverage"].to_numpy() - target_coverage
                )
            )
        ),
...
def main() -> None:
    ...
    local_parameters = yaml.safe_load(
        LOCAL_PARAMETER_PATH.read_text(encoding="utf-8")
    )
    ...
    if sensitivity_coverages != [0.5, 0.9]:
        raise AssertionError(
            f"Unexpected sensitivity coverages: {sensitivity_coverages}"
        )
    ...
    if bias_limit != 0.5 or allowed_violation != 0.02:
        raise AssertionError(
            f"Unexpected controller limits: {bias_limit}, {allowed_violation}"
        )
    ...
    missing_columns = [
        name for name in required_columns if name not in calibration.columns
    ]
    if missing_columns:
    ...
    observed_splits = set(calibration["split"].astype(str).unique().tolist())
    ...
    if set(calibration["dataset_id"].astype(str).unique()) != {DATASET_ID}:
        raise AssertionError("Calibration score dataset_id is invalid")

    calibration["series_id"] = calibration["series_id"].astype(str)
    first_appearance = calibration["series_id"].drop_duplicates().tolist()
    calibration["_series_order"] = pd.Categorical(
        calibration["series_id"], categories=first_appearance, ordered=True
    )
    calibration = (
    ...
    if not (
        len(calibration) == EXPECTED_ROWS
        and len(per_series_lengths) == EXPECTED_SERIES
        and int(per_series_lengths.min()) == EXPECTED_ROWS_PER_SERIES
        and int(per_series_lengths.max()) == EXPECTED_ROWS_PER_SERIES
    ):
        raise AssertionError("Electricity calibration dimensions changed")
    per_series_contiguous = True
    per_series_hourly = True
    for _, group in calibration.groupby(
        "series_id", sort=False, observed=True
    ):
        indices = group["time_index"].to_numpy(dtype=np.int64)
        per_series_contiguous = per_series_contiguous and bool(
            np.all(np.diff(indices) == 1)
        )
        timestamps = pd.to_datetime(group["timestamp"]).to_numpy(
            dtype="datetime64[ns]"
        )
        per_series_hourly = per_series_hourly and bool(
            np.all(np.diff(timestamps) == np.timedelta64(1, "h"))
        )

    probability = calibration["black_box_probability"].to_numpy(
        dtype=np.float64
    )
    ...
    scales = calibration["seasonal_naive_mae_scale"].to_numpy(
        dtype=np.float64
    )
    scales_positive = bool(np.isfinite(scales).all() and np.all(scales > 0.0))

    all_target_coverages = sorted(
        set([primary_coverage] + sensitivity_coverages)
    )
    ...
    tuning = pd.DataFrame(candidate_summaries).sort_values(
        "eta", kind="stable"
    ).reset_index(drop=True)
    ...
    if feasible.empty:
        ...
        ranked = tuning.sort_values(
            [
                ...
                "worst_48h_weighted_coverage_violation",
                "eta",
            ],
            kind="stable",
        )
    ...
    static_summary, static_trace, _ = simulate_controller(
        calibration,
        0.0,
        primary_threshold,
        primary_coverage,
    ...
    trace_coverage = float(
        selected_trace["simple_count"].sum()
        / selected_trace["batch_rows"].sum()
    )
    ...
    checks_items: list[tuple[str, bool, str]] = [
        (
            "upstream_local_weighting_checks_passed",
            bool(passed_column(local_checks["passed"]).all()),
            f"checks={len(local_checks)}",
        ),
        (
            "input_contains_only_calibration",
            observed_splits == {"calibration"},
            str(observed_splits),
        ),
        (
            "test_not_accessed",
            "test" not in observed_splits,
            str(observed_splits),
        ),
        (
        ...
        (
            ...
            calibration["series_id"].nunique() == EXPECTED_SERIES,
            f"series={calibration['series_id'].nunique()}",
        ),
        (
            "per_series_time_indices_are_contiguous",
            per_series_contiguous,
            str(per_series_contiguous),
        ),
        (
            "per_series_timestamps_are_hourly",
            per_series_hourly,
            str(per_series_hourly),
        ),
        (
            "expected_relative_time_points",
            calibration["relative_calibration_step"].nunique()
            == EXPECTED_RELATIVE_STEPS,
            (
                f"steps={calibration['relative_calibration_step'].nunique()}"
            ),
        ),
        (
            "expected_per_series_length_range",
            int(per_series_lengths.min()) == EXPECTED_ROWS_PER_SERIES
            and int(per_series_lengths.max()) == EXPECTED_ROWS_PER_SERIES,
            (
                f"minimum={per_series_lengths.min()}; "
                f"maximum={per_series_lengths.max()}"
            ),
        ),
        (
            "every_relative_step_contains_all_321_series",
            int(selected_trace["batch_rows"].min()) == EXPECTED_BATCH_ROWS
            and int(selected_trace["batch_rows"].max()) == EXPECTED_BATCH_ROWS,
            (
                f"minimum={selected_trace['batch_rows'].min()}; "
                f"maximum={selected_trace['batch_rows'].max()}"
            ),
        ),
        (
            "routing_probabilities_are_valid",
            probability_valid,
            f"minimum={probability.min():.6f}; maximum={probability.max():.6f}",
        ),
        (
            "loss_scales_are_positive",
            scales_positive,
            f"minimum={scales.min():.6g}",
        ),
        (
            "three_coverage_thresholds_saved",
            len(thresholds) == 3,
            f"rows={len(thresholds)}",
        ),
        (
            "primary_static_quantile_matches_target",
            abs(expected_primary_static_coverage - primary_coverage)
            <= 1.0 / len(calibration),
            f"coverage={expected_primary_static_coverage:.9f}",
        ),
        (
            "four_eta_candidates_evaluated",
            len(tuning) == len(eta_grid),
            f"rows={len(tuning)}",
        ),
        (
            ...
            bool(
                np.isfinite(
                    tuning[finite_tuning_columns].to_numpy(dtype=float)
                ).all()
            ),
            f"candidates={len(tuning)}",
        ),
        (
            "selected_eta_is_preregistered",
            selected_eta in eta_grid,
            f"eta={selected_eta}",
        ),
        (
            "selected_controller_obeys_constraint_when_feasible",
            bool(
                feasible.empty
                or float(selected_summary["absolute_coverage_violation"])
                <= allowed_violation
            ),
            (
                "violation="
                f"{float(selected_summary['absolute_coverage_violation']):.6f}"
            ),
        ),
        (
            "trace_has_one_row_per_relative_hour",
            len(selected_trace) == EXPECTED_RELATIVE_STEPS,
            f"rows={len(selected_trace)}",
        ),
        (
            "decision_rows_match_calibration",
            len(selected_decisions) == len(calibration)
            and not selected_decisions.duplicated(
                ["series_id", "time_index"]
            ).any(),
            f"rows={len(selected_decisions)}",
        ),
        (
            "decision_and_trace_coverage_agree",
            abs(decision_coverage - trace_coverage) <= 1e-15,
            (
                f"decision={decision_coverage:.12f}; "
                f"trace={trace_coverage:.12f}"
            ),
        ),
        (
            "equal_series_and_pooled_losses_agree_for_equal_lengths",
            abs(
                float(selected_summary["equal_series_mean_scaled_loss"])
                - float(selected_summary["pooled_mean_scaled_loss"])
            )
            <= 1e-12,
            (
                f"equal_series={float(selected_summary['equal_series_mean_scaled_loss']):.12f}; "
                f"pooled={float(selected_summary['pooled_mean_scaled_loss']):.12f}"
            ),
        ),
        (
            "controller_update_uses_decisions_only",
            True,
            "bias_next=clip(bias+eta*(target-batch_coverage))",
        ),
    ]
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly coverage calibration failed: {message}")
    ...
    selected_decisions["dataset_id"] = selected_decisions["dataset_id"].astype(
        "category"
    )
    selected_decisions["series_id"] = selected_decisions["series_id"].astype(
        "category"
    )
    selected_decisions.to_parquet(
        DECISION_PATH, index=False, compression="snappy"
    )
    ...
    selected_parameters = {
        ...
        "rolling_diagnostic_window_hours": ROLLING_WINDOW_HOURS,
        ...
        "selection_metric": (
            "minimum equal-series scaled loss among pooled-coverage-feasible candidates"
        ),
        "candidate_eta_values": eta_grid,
        "calibration_rows": len(calibration),
        "calibration_series": int(calibration["series_id"].nunique()),
        "calibration_relative_steps": int(
            calibration["relative_calibration_step"].nunique()
        ),
        "achieved_calibration_simple_coverage": float(
            selected_summary["achieved_simple_coverage"]
        ),
        "calibration_absolute_violation": float(
            selected_summary["absolute_coverage_violation"]
        ),
        "calibration_equal_series_mean_scaled_loss": float(
            selected_summary["equal_series_mean_scaled_loss"]
        ),
        "calibration_pooled_mean_scaled_loss": float(
            selected_summary["pooled_mean_scaled_loss"]
        ),
        "calibration_worst_48h_weighted_coverage_violation": float(
            selected_summary["worst_48h_weighted_coverage_violation"]
        ),
        "calibration_final_bias": float(selected_summary["final_bias"]),
        "static_calibration_simple_coverage": float(
            static_summary["achieved_simple_coverage"]
        ),
        "static_calibration_equal_series_mean_scaled_loss": float(
            static_summary["equal_series_mean_scaled_loss"]
        ),
        "static_calibration_pooled_mean_scaled_loss": float(
            static_summary["pooled_mean_scaled_loss"]
        ),
        "thresholds": [
            {
                ...
                "achieved_static_calibration_coverage": float(
                    row.achieved_static_calibration_coverage
                ),
            }
    ...
    axes[0].hist(
        calibration["black_box_probability"],
        bins=45,
        color="#4C78A8",
        alpha=0.82,
    )
    ...
    axes[1].plot(
        ...
        alpha=0.28,
        linewidth=0.7,
        label="Relative-hour batch coverage",
    )
    axes[1].plot(
        selected_trace["relative_calibration_step"],
        selected_trace["rolling_48h_simple_coverage"],
        color="#E15759",
        linewidth=1.6,
        label="Weighted 48-hour coverage",
    )
    axes[1].axhline(
        primary_coverage,
        color="black",
    ...
    axes[1].set_xlabel("Hours since entering calibration")
    ...
    axes[2].set_xlabel("Hours since entering calibration")
    axes[2].set_ylabel("Probability threshold")
    axes[2].legend(frameon=False)

    figure.suptitle(
        "Electricity Hourly calibration-only coverage-controller selection", fontsize=15
    )
    ...
    report = {
        ...
        "selected_absolute_violation": float(
            selected_summary["absolute_coverage_violation"]
        ),
        "selected_equal_series_scaled_loss": float(
            selected_summary["equal_series_mean_scaled_loss"]
        ),
        "static_equal_series_scaled_loss": float(
            static_summary["equal_series_mean_scaled_loss"]
        ),
        "selected_worst_48h_violation": float(
            selected_summary["worst_48h_weighted_coverage_violation"]
        ),
        "coverage_constraint_passed": bool(
            float(selected_summary["absolute_coverage_violation"])
            <= allowed_violation
        ),
    ...
    print()
    print("Electricity Hourly 覆盖率控制器校准全部通过")
    print("参数选择数据范围：仅 calibration")
    print("模型是否重新训练：否")
    print("test 是否访问：否")
    print("时间对齐：各序列进入 calibration 后的相对小时")
    print("calibration 样本数量：", len(calibration))
    print("序列数量：", calibration["series_id"].nunique())
    print(
        "每条序列 calibration 长度范围：",
        f"{per_series_lengths.min()} 至 {per_series_lengths.max()} 小时",
    )
    print(
        "相对时间点数量：",
        calibration["relative_calibration_step"].nunique(),
    ...
    print(
        "每个相对时点活动序列数量范围：",
        f"{selected_summary['minimum_batch_rows']} 至 "
        f"{selected_summary['maximum_batch_rows']}",
    )
    ...
    print(
        "自适应实际覆盖率：",
        f"{float(selected_summary['achieved_simple_coverage']):.6f}",
    )
    print(
        "绝对覆盖率偏差：",
        f"{float(selected_summary['absolute_coverage_violation']):.6f}",
    ...
    print(
        "自适应等权逐序列缩放损失：",
        f"{float(selected_summary['equal_series_mean_scaled_loss']):.6f}",
    )
    print(
        "静态阈值等权逐序列缩放损失：",
        f"{float(static_summary['equal_series_mean_scaled_loss']):.6f}",
    )
    print(
        "自适应汇总缩放损失：",
        f"{float(selected_summary['pooled_mean_scaled_loss']):.6f}",
    )
    print(
        "最差48小时加权覆盖率偏差：",
        f"{float(selected_summary['worst_48h_weighted_coverage_violation']):.6f}",
    )
    print("最终 bias：", f"{float(selected_summary['final_bias']):.9f}")
    print(
        "覆盖率约束：",
        "通过"
        if float(selected_summary["absolute_coverage_violation"])
        <= allowed_violation
        else "未通过",
    )
