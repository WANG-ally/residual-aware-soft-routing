#!/usr/bin/env python3
"""Train Electricity Hourly pretest routing baselines and ablations.
...
FOLD_MANIFEST_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_soft_router_fold_manifest.csv"
)
FOLD_SAMPLE_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_soft_router_sample_positions.parquet"
)
FINAL_SAMPLE_PATH = (
    PROJECT_ROOT / "results/electricity_hourly_final_router_sample_positions.parquet"
)
CONFIG_PATH = PROJECT_ROOT / "experiment_config.yaml"
FREEZE_PATH = PROJECT_ROOT / "results/electricity_hourly_pretest_freeze_manifest.json"

PROTOCOL_PATH = OUTPUT_ROOT / "results/electricity_hourly_baseline_protocol_supplement.yaml"
DETAIL_PATH = OUTPUT_ROOT / "results/electricity_hourly_baseline_rolling_validation.csv"
SUMMARY_PATH = OUTPUT_ROOT / "results/electricity_hourly_baseline_tuning_summary.csv"
SELECTED_PATH = OUTPUT_ROOT / "results/electricity_hourly_selected_baseline_params.yaml"
CALIBRATION_PATH = (
    OUTPUT_ROOT / "results/electricity_hourly_baseline_calibration_scores.parquet"
)
CHECKS_PATH = OUTPUT_ROOT / "results/electricity_hourly_baseline_training_checks.csv"
FIGURE_PATH = OUTPUT_ROOT / "figures/electricity_hourly_baseline_validation.png"
REPORT_PATH = OUTPUT_ROOT / "logs/electricity_hourly_baseline_training_report.json"
...
EXPECTED_SERIES = 321
EXPECTED_FREEZE_ID = "1121502578b7777324fc9c51211874905ea41d17bc7e236fc63a915a5e84da9f"
...
def main() -> None:
    ...
    protected_outputs = [
        SELECTED_PATH,
        CALIBRATION_PATH,
        MODEL_DIR / "electricity_hourly_hard_logistic_same_features.joblib",
        MODEL_DIR / "electricity_hourly_hard_random_forest_same_features.joblib",
        MODEL_DIR / "electricity_hourly_soft_targets_only.joblib",
        MODEL_DIR / "electricity_hourly_residual_features_only.joblib",
        MODEL_DIR / "electricity_hourly_class_weight_only.joblib",
    ]
    ...
    if (
        ...
        or len(residual_features) != 2
        or len(full_features) != 16
        or full_features != context_features + residual_features
        or not set(residual_features).issubset(available_residual_features)
    ):
        raise AssertionError("Electricity Hourly selected baseline feature sets are invalid")
    ...
    protocol = {
        ...
        "fold_boundaries": "identical to electricity_hourly_soft_router_fold_manifest.csv",
        "training_sample": {
            "maximum_rows_per_fold": MAX_TRAINING_SAMPLE_ROWS,
            "sample_count_rule": "min(all available prefix rows, 100000)",
            "positions": "identical to Electricity Hourly full-router tuning",
            "final_positions": "identical to Electricity Hourly final full-router fit",
    ...
    if not (
        ...
        and int(router_series_lengths.min()) == EXPECTED_ROUTER_ROWS_PER_SERIES
        and int(router_series_lengths.max()) == EXPECTED_ROUTER_ROWS_PER_SERIES
    ):
        raise AssertionError("Electricity router_train dimensions changed")
    ...
    for fold in range(1, N_FOLDS + 1):
        ...
        partial_path = DETAIL_PATH.with_name(
            "electricity_hourly_baseline_rolling_validation.partial.csv"
        )
    ...
    for method, (feature_names, model_kind) in model_definitions.items():
        ...
        model_path = MODEL_DIR / f"electricity_hourly_{method}.joblib"
        bundle = {
            "dataset_id": DATASET_ID,
            "method": method,
            "model": model,
    ...
    formal_outputs = [
        PROJECT_ROOT / "results/electricity_hourly_test_predictions.parquet",
        PROJECT_ROOT / "results/electricity_hourly_test_aggregate_metrics.csv",
        PROJECT_ROOT / "logs/electricity_hourly_formal_test_access_receipt.json",
    ]
    ...
    check_items: list[tuple[str, bool, str]] = [
        ...
        (
            "equal_length_router_and_calibration_dimensions_match",
            len(router_series_lengths) == EXPECTED_SERIES
            and int(router_series_lengths.min()) == EXPECTED_ROUTER_ROWS_PER_SERIES
            and int(router_series_lengths.max()) == EXPECTED_ROUTER_ROWS_PER_SERIES
        ...
        (
            ...
            len(model_hashes) == 5
            and all(
                (MODEL_DIR / f"electricity_hourly_{method}.joblib").is_file()
    ...
    if not failed.empty:
        ...
        raise AssertionError(f"Electricity Hourly baseline training failed: {message}")
    ...
    figure.suptitle(
        "Electricity Hourly pretest baseline and ablation selection", fontsize=14
    )
    ...
    print()
    print("Electricity Hourly 预测试基线与消融模型训练全部通过")
    print("第一次冻结完整性检查：通过")
